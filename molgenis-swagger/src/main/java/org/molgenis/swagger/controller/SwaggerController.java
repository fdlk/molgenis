...
public class SwaggerController extends PluginController {
  ...
  /**
   * Serves the Swagger UI which allows you to try out the documented endpoints. Sets the url
   * parameter to the swagger yaml that describes the REST API. Creates an apiKey token for the
   * current user.
   */
  ...
  public String init(Model model) {
    model.addAttribute(
        ...
        ServletUriComponentsBuilder.fromCurrentContextPath()
            .path(URI + "/swagger.yml")
            .toUriString());
    final String currentUsername = SecurityUtils.getCurrentUsername();
    if (currentUsername != null) {
      model.addAttribute(
          "token", tokenService.generateAndStoreToken(currentUsername, "For Swagger UI"));
    }
