...
public class PlatformIT extends AbstractTestNGSpringContextTests {
  ...
  @Autowired private LanguageMetadata languageMetadata;
  @Autowired private EntityTypeMetadata entityTypeMetadata;
  ...
  static List<GrantedAuthority> makeAuthorities(
      String entityTypeId, boolean write, boolean read, boolean count) {
  ...
  public void testDeletePackage() {
    runAsSystem(
        () -> {
          ...
          Package subPackage =
              packageFactory.create("parent_sub").setLabel("sub").setParent(parentPackage);
          metadataService.upsertPackages(Stream.of(parentPackage, subPackage));

          EntityType entityTypeInSubPackage =
              testHarness.createDynamicRefEntityType("entityInSub", subPackage);
          EntityType entityTypeInParentPackage =
              testHarness.createDynamicTestEntityType(
                  "entityInParent", parentPackage, entityTypeInSubPackage);

          metadataService.upsertEntityTypes(
              asList(entityTypeInSubPackage, entityTypeInParentPackage));

          List<Entity> entities =
              createAndAdd(entityTypeInParentPackage, entityTypeInSubPackage, 5);
          Set<Entity> refEntities =
              entities.stream().map(e -> e.getEntity(ATTR_XREF)).collect(toSet());
    ...
    runAsSystem(
        () -> {
          ...
          EntityType refEntityType =
              testHarness.createDynamicRefEntityType("entityType_onetomany", package_);
          EntityType entityType =
              testHarness.createDynamicTestEntityType(
                  "refEntityType_onetomany", package_, refEntityType);
  ...
  public void testReindexOnEntityTypeUpdate() {
    runAsSystem(
        () -> {
          ...
          List<Entity> entities =
              testHarness.createSelfRefEntitiesWithEmptyReferences(entityType, 3).collect(toList());
  ...
  public void testFindAllTyped() {
    ...
    Supplier<Stream<Entity>> retrieved =
        () -> dataService.findAll(entityTypeDynamic.getId(), Entity.class);
    ...
    Supplier<Stream<TestEntityStatic>> retrieved =
        () ->
            dataService.findAll(
                ...
                Stream.concat(entities.stream().map(Entity::getIdValue), of("bogus")),
  ...
  public void testFindAllStreamFetch() {
    ...
    Stream<Entity> retrieved =
        dataService.findAll(entityTypeDynamic.getId(), ids, new Fetch().field(ATTR_ID));
    assertEquals(retrieved.count(), entities.size());
  }
  ...
  public void testFindQueryOperatorEq(
    ...
    Supplier<Stream<Entity>> found =
        () -> dataService.query(entityTypeDynamic.getId()).eq(attrName, value).findAll();
  ...
  public void testFindQueryOperatorIn(List<String> ids, List<Integer> expectedEntityIndices) {
    List<Entity> entities = createDynamicAndAdd(2);
    Supplier<Stream<Entity>> found =
        () -> dataService.query(entityTypeDynamic.getId()).in(ATTR_ID, ids).findAll();
    List<Entity> foundAsList = found.get().collect(toList());
    assertEquals(foundAsList.size(), expectedEntityIndices.size());
    for (int i = 0; i < expectedEntityIndices.size(); ++i) {
      assertTrue(
  ...
  public void testFindQueryOperatorLess(int value, List<Integer> expectedEntityIndices) {
    List<Entity> entities = createDynamicAndAdd(5);
    Supplier<Stream<Entity>> found =
        () -> dataService.query(entityTypeDynamic.getId()).lt(ATTR_INT, value).findAll();
    List<Entity> foundAsList = found.get().collect(toList());
    assertEquals(foundAsList.size(), expectedEntityIndices.size());
    for (int i = 0; i < expectedEntityIndices.size(); ++i) {
      assertTrue(
  ...
  public void testFindQueryOperatorLessEqual(int value, List<Integer> expectedEntityIndices) {
    List<Entity> entities = createDynamicAndAdd(5);
    Supplier<Stream<Entity>> found =
        () -> dataService.query(entityTypeDynamic.getId()).le(ATTR_INT, value).findAll();
    List<Entity> foundAsList = found.get().collect(toList());
    assertEquals(foundAsList.size(), expectedEntityIndices.size());
    for (int i = 0; i < expectedEntityIndices.size(); ++i) {
      assertTrue(
  ...
  public void testFindQueryOperatorGreater(int value, List<Integer> expectedEntityIndices) {
    List<Entity> entities = createDynamicAndAdd(3);
    Supplier<Stream<Entity>> found =
        () -> dataService.query(entityTypeDynamic.getId()).gt(ATTR_INT, value).findAll();
    List<Entity> foundAsList = found.get().collect(toList());
    assertEquals(foundAsList.size(), expectedEntityIndices.size());
    for (int i = 0; i < expectedEntityIndices.size(); ++i) {
      assertTrue(
  ...
  public void testFindQueryOperatorGreaterEqual(int value, List<Integer> expectedEntityIndices) {
    List<Entity> entities = createDynamicAndAdd(3);
    Supplier<Stream<Entity>> found =
        () -> dataService.query(entityTypeDynamic.getId()).ge(ATTR_INT, value).findAll();
    List<Entity> foundAsList = found.get().collect(toList());
    assertEquals(foundAsList.size(), expectedEntityIndices.size());
    for (int i = 0; i < expectedEntityIndices.size(); ++i) {
      assertTrue(
  ...
  public void testFindQueryOperatorRange(int low, int high, List<Integer> expectedEntityIndices) {
    List<Entity> entities = createDynamicAndAdd(3);
    Supplier<Stream<Entity>> found =
        () -> dataService.query(entityTypeDynamic.getId()).rng(ATTR_INT, low, high).findAll();
    List<Entity> foundAsList = found.get().collect(toList());
    assertEquals(foundAsList.size(), expectedEntityIndices.size());
    for (int i = 0; i < expectedEntityIndices.size(); ++i) {
      assertTrue(
  ...
  public void testFindQueryOperatorLike(String likeStr, List<Integer> expectedEntityIndices) {
    List<Entity> entities = createDynamicAndAdd(2);
    Supplier<Stream<Entity>> found =
        () -> dataService.query(entityTypeDynamic.getId()).like(ATTR_STRING, likeStr).findAll();
    List<Entity> foundAsList = found.get().collect(toList());
    assertEquals(foundAsList.size(), expectedEntityIndices.size());
    for (int i = 0; i < expectedEntityIndices.size(); ++i) {
      assertTrue(
  ...
  public void testFindQueryOperatorNot(int value, List<Integer> expectedEntityIndices) {
    List<Entity> entities = createDynamicAndAdd(3);
    Supplier<Stream<Entity>> found =
        () -> dataService.query(entityTypeDynamic.getId()).not().eq(ATTR_INT, value).findAll();
    List<Entity> foundAsList = found.get().collect(toList());
    assertEquals(foundAsList.size(), expectedEntityIndices.size());
    for (int i = 0; i < expectedEntityIndices.size(); ++i) {
      assertTrue(
  ...
  public void testFindQueryOperatorSearch(String searchStr, List<Integer> expectedEntityIndices) {
    List<Entity> entities = createDynamicAndAdd(2);
    Supplier<Stream<Entity>> found =
        () -> dataService.query(entityTypeDynamic.getId()).search(ATTR_HTML, searchStr).findAll();
    List<Entity> foundAsList = found.get().collect(toList());
    assertEquals(foundAsList.size(), expectedEntityIndices.size());
    for (int i = 0; i < expectedEntityIndices.size(); ++i) {
      assertTrue(
  ...
  public void testFindQueryLimit2_Offset2_sortOnInt() {
    ...
    List<Entity> testEntities =
        testHarness.createTestEntities(entityTypeDynamic, 10, testRefEntities).collect(toList());
    runAsSystem(
        () -> {
  ...
  public void testFindQueryTypedStatic() {
    List<Entity> entities = createStaticAndAdd(5);
    Supplier<Stream<TestEntityStatic>> found =
        () ->
            dataService.findAll(
                entityTypeStatic.getId(),
                new QueryImpl<TestEntityStatic>().eq(ATTR_ID, entities.get(0).getIdValue()),
                TestEntityStatic.class);
  ...
  public void testFindOneFetch() {
    ...
    assertNotNull(
        dataService.findOneById(
            entityTypeDynamic.getId(), entity.getIdValue(), new Fetch().field(ATTR_ID)));
  }
  ...
  public void testFindOneQuery() {
    ...
    entity =
        dataService.findOne(
            entityTypeDynamic.getId(), new QueryImpl<>().eq(ATTR_ID, entity.getIdValue()));
  ...
  public void testIterator() {
    ...
    StreamSupport.stream(dataService.spliterator(), false)
        .forEach(repo -> LOG.info(repo.getName()));
  ...
  public void testCreateSelfXref() {
    Entity entitySelfXref =
        entitySelfXrefTestHarness
            .createTestEntities(selfXrefEntityType, 1)
            .collect(toList())
            .get(0);
    ...
    waitForIndexToBeStable(selfXrefEntityType, indexService, LOG);
    Entity entity =
        dataService.findOneById(selfXrefEntityType.getId(), entitySelfXref.getIdValue());
  ...
  public void testIndexUpdateMetaDataRemoveAttribute() {
    IndexMetadataCUDOperationsPlatformIT.testIndexUpdateMetaDataRemoveAttribute(
        entityTypeDynamic,
        EntityTestHarness.ATTR_CATEGORICAL,
        searchService,
        metaDataService,
        indexService);
    ...
    IndexMetadataCUDOperationsPlatformIT.testIndexUpdateMetaDataRemoveAttribute(
        entityTypeDynamic,
        EntityTestHarness.ATTR_DATETIME,
        searchService,
        metaDataService,
        indexService);
    ...
    IndexMetadataCUDOperationsPlatformIT.testIndexUpdateMetaDataRemoveAttribute(
        entityTypeDynamic,
        EntityTestHarness.ATTR_DECIMAL,
        searchService,
        metaDataService,
        indexService);
    ...
    IndexMetadataCUDOperationsPlatformIT.testIndexUpdateMetaDataRemoveAttribute(
        entityTypeDynamic,
        EntityTestHarness.ATTR_HYPERLINK,
        searchService,
        metaDataService,
        indexService);
    ...
    IndexMetadataCUDOperationsPlatformIT.testIndexUpdateMetaDataRemoveAttribute(
        entityTypeDynamic,
        EntityTestHarness.ATTR_COMPOUND,
        searchService,
        metaDataService,
        indexService);
  }
  ...
  public void addAndDeleteSingleAttribute() {
    ...
    Query<Entity> q0 =
        new QueryImpl<>()
            .eq(NEW_ATTRIBUTE, "NEW_ATTRIBUTE_0")
            .or()
    ...
    runAsSystem(
        () -> {
          List expected =
              dataService
                  .findAll(entityTypeDynamic.getId(), q0)
  ...
  public void addStreamAttribute() {
    ...
    runAsSystem(
        () -> {
          ...
          Attribute attribute =
              dataService.findOneById(ATTRIBUTE_META_DATA, newAttr.getIdValue(), Attribute.class);
  ...
  public void addAndDeleteSingleAttributeStream() {
    ...
    runAsSystem(
        () -> {
          dataService.update(
              ENTITY_TYPE_META_DATA, Stream.of(entityType)); // Adds the column to the table
  ...
  public void testOneDimensionalAggregateQuery() {
    ...
    AggregateResult expectedResult =
        new AggregateResult(
            asList(singletonList(3L), singletonList(3L)), asList(0L, 1L), emptyList());
  ...
  public void testTwoDimensionalAggregateQuery() {
    ...
    AggregateResult expectedResult =
        new AggregateResult(
            asList(asList(0L, 3L), asList(3L, 0L)), asList(0L, 1L), asList("option1", "option2"));
    assertEquals(result, expectedResult);
  }
  ...
  public void testAggregateQueryWithQuery() {
    ...
    AggregateResult expectedResult =
        new AggregateResult(
            asList(asList(0L, 2L), asList(1L, 0L)), asList(0L, 1L), asList("option1", "option2"));
    assertEquals(result, expectedResult);
  }
