 ...
 *
 * <p>Ensures that when an Entity is created the owner is set to the current user, users can only
 * view, update, delete their own entities.
 *
 * <p>Admins are not effected.
 */
public class OwnedEntityRepositoryDecorator extends AbstractRepositoryDecorator<Entity> {
            ...
            entities -> {
              if (mustAddRowLevelSecurity()) {
                ...
                consumer.accept(
                    entities
                        .stream()
                        .filter(OwnedEntityRepositoryDecorator::currentUserIsOwner)
                        .collect(toList()));
              } else {
  ...
  public void update(Entity entity) {
    if (isOwnedEntityType()
        && (mustAddRowLevelSecurity() || entity.get(OwnedEntityType.OWNER_USERNAME) == null))
  ...
  private static boolean currentUserIsOwner(Entity entity) {
    return null != entity
        && Objects.equals(SecurityUtils.getCurrentUsername(), getOwnerUserName(entity));
  }
}
