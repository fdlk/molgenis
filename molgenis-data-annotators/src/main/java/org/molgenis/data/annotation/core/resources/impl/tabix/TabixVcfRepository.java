...
public class TabixVcfRepository extends VcfRepository {
  ...
  TabixVcfRepository(
      VcfReaderFactory readerFactory,
  ...
  /**
   * Examines a {@link Query} and finds the first {@link QueryRule} with operator {@link
   * Operator#EQUALS} whose field matches an attributeName. It returns the value of that first
   * matching {@link QueryRule}.
   *
  ...
  private static Object getFirstEqualsValueFor(String attributeName, Query<Entity> q) {
    return q.getRules()
        .stream()
        .filter(
            rule -> attributeName.equals(rule.getField()) && rule.getOperator() == Operator.EQUALS)
        .findFirst()
        .get()
        .getValue();
  }
  ...
  public synchronized List<Entity> query(String chrom, int posFrom, int posTo) {
    String queryString =
        String.format("%s:%s-%s", checkNotNull(chrom), checkNotNull(posFrom), checkNotNull(posTo));
    try {
      ...
      LOG.trace(
          "Unable to read from tabix resource for query: "
              + queryString
   ...
   * </pre>
   *
   * <p>returns 2 variants:
   *
   * <ul>
   *   <li>"1 1115547 . CG C,TG"
   *   <li>"1 1115548 rs114390380 G A"
   * </ul>
   *
