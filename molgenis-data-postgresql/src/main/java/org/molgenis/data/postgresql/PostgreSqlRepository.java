...
import static org.molgenis.data.postgresql.PostgreSqlQueryGenerator.getSqlInsertJunction;
 ...
 *
 * <ul>
 *   <li>Attributes with expression are not persisted
 *   <li>Cross-backend attribute references are supported
 *   <li>Query operators DIS_MAX, FUZZY_MATCH, FUZZY_MATCH_NGRAM, SEARCH, SHOULD are not supported
 * </ul>
 */
class PostgreSqlRepository extends AbstractRepository {
  ...
  public long count(Query<Entity> q) {
    ...
    return jdbcTemplate.queryForObject(
        sql, parameters.toArray(new Object[parameters.size()]), Long.class);
  }
  ...
  private Object processResultSet(
  ...
  /**
   * Handles a batch of Entities. Looks up the values for MREF ID attributes and sets them as
   * references in the entities. Then feeds the entities to the {@link Consumer}
   *
  ...
  /**
   * Selects MREF IDs for an MREF attribute from the junction table, in the order of the MREF
   * attribute value.
   *
   ...
   * @param ids {@link Set} of {@link Object}s containing the values for the ID attribute of the
   *     entity
   * @param refIdDataType {@link AttributeType} of the ID attribute of the refEntity of the
   *     attribute
   * @return Multimap mapping entity ID to a list containing the MREF IDs for the values in the
   *     attribute
   */
  private Multimap<Object, Object> selectMrefIDsForAttribute(
      EntityType entityType,
    ...
    jdbcTemplate.query(
        ...
        row -> {
          ...
          switch (idAttributeDataType) {
            ...
            default:
              throw new RuntimeException(
                  format("Unexpected id attribute type [%s]", idAttributeDataType));
          }
          ...
          switch (refIdDataType) {
            ...
            default:
              throw new RuntimeException(
                  format("Unexpected id attribute type [%s]", refIdDataType));
          }
    ...
    if (LOG.isTraceEnabled())
      LOG.trace(
          "Selected {} ID values for MREF attribute {} in {}",
          mrefIDs.values().stream().collect(counting()),
          mrefAttr.getName(),
          stopwatch);
    return mrefIDs;
  }
  ...
  private Integer addBatching(Iterator<? extends Entity> entities) {
    ...
    final List<Attribute> junctionTableAttrs =
        getJunctionTableAttributes(entityType).collect(toList());
    final String insertSql = getSqlInsert(entityType);

    Iterators.partition(entities, BATCH_SIZE)
        .forEachRemaining(
            entitiesBatch -> {
              if (LOG.isDebugEnabled()) {
              ...
              jdbcTemplate.batchUpdate(
                  insertSql, new BatchAddPreparedStatementSetter(entitiesBatch, tableAttrs));

              // persist values in entity junction table
              if (!junctionTableAttrs.isEmpty()) {
                Map<String, List<Map<String, Object>>> mrefs =
                    createMrefMap(idAttr, junctionTableAttrs, entitiesBatch);
  ...
  private static Map<String, List<Map<String, Object>>> createMrefMap(
      Attribute idAttr, List<Attribute> junctionTableAttrs, List<? extends Entity> entitiesBatch) {
    Map<String, List<Map<String, Object>>> mrefs =
        Maps.newHashMapWithExpectedSize(junctionTableAttrs.size());
        ...
        if (!attr.isNillable() && Iterables.isEmpty(refEntities)) {
          throw new MolgenisValidationException(
              new ConstraintViolation(
                  String.format(
                      "The attribute [%s] of entity [%s] with id [%s] can not be null.",
  ...
  private void updateBatching(Iterator<? extends Entity> entities) {
    ...
    final List<Attribute> tableAttrs = getTableAttributes(entityType).collect(toList());
    final List<Attribute> junctionTableAttrs =
        getJunctionTableAttributes(entityType).filter(attr -> !attr.isReadOnly()).collect(toList());
  ...
  private void verifyUpdate(List<? extends Entity> entitiesBatch, int[] counts, Attribute idAttr) {
    ...
    if (nrUpdatedEntities < entitiesBatch.size()) {
      ...
      throw new MolgenisValidationException(
          new ConstraintViolation(
              format(
                  "Cannot update [%s] with id [%s] because it does not exist",
                  entityType.getId(), nonExistingEntityId.toString())));
    }
  }
  ...
  private void addMrefs(final List<Map<String, Object>> mrefs, final Attribute attr) {
    ...
    if (!attr.isNillable() && mrefs.isEmpty()) {
      throw new MolgenisValidationException(
          new ConstraintViolation(
              format(
                  "Entity [%s] attribute [%s] value cannot be null",
                  entityType.getId(), attr.getName())));
    }
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          "Adding junction table entries for entity [{}] attribute [{}]",
          getName(),
          attr.getName());
      if (LOG.isTraceEnabled()) {
    ...
    jdbcTemplate.batchUpdate(
        insertMrefSql, new BatchJunctionTableAddPreparedStatementSetter(mrefs, attr, idAttr));
  }
  ...
  private void removeMrefs(final List<Object> ids, final Attribute attr) {
    ...
    String deleteMrefSql =
        getSqlDelete(PostgreSqlNameGenerator.getJunctionTableName(entityType, attr), idAttr);

    if (LOG.isDebugEnabled()) {
      LOG.debug(
          "Removing junction table entries for entity [{}] attribute [{}]",
          getName(),
          attr.getName());
      if (LOG.isTraceEnabled()) {
    ...
    jdbcTemplate.batchUpdate(
        deleteMrefSql, new BatchJunctionTableDeletePreparedStatementSetter(ids));
  }
  ...
  private static class BatchUpdatePreparedStatementSetter implements BatchPreparedStatementSetter {
    ...
    BatchUpdatePreparedStatementSetter(
        List<? extends Entity> entities, List<Attribute> tableAttrs, Attribute idAttr) {
      this.entities = entities;
      this.tableAttrs = tableAttrs;
      this.idAttr = idAttr;
    }
  ...
  private static class BatchJunctionTableAddPreparedStatementSetter
    ...
    BatchJunctionTableAddPreparedStatementSetter(
        List<Map<String, Object>> mrefs, Attribute attr, Attribute idAttr) {
      this.mrefs = mrefs;
      this.attr = attr;
      this.idAttr = idAttr;
    }
