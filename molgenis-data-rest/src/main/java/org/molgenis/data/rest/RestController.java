 ...
 *
 * <p>Query, create, update and delete entities.
 *
 * <p>If a repository isn't capable of doing the requested operation an error is thrown.
 *
 * <p>Response is json.
 *
 * @author erwin
...
public class RestController {
  ...
  public EntityTypeResponse retrieveEntityType(
    ...
    return new EntityTypeResponse(
        meta, attributeSet, attributeExpandSet, permissionService, dataService, languageService);
  }

  /**
   * Same as retrieveEntityType (GET) only tunneled through POST.
   *
   * <p>Example url: /api/v1/person/meta?_method=GET
   *
   * @return EntityType
   */
  @RequestMapping(
  ...
  public EntityTypeResponse retrieveEntityTypePost(
    ...
    EntityType meta = dataService.getEntityType(entityTypeId);
    return new EntityTypeResponse(
        meta, attributesSet, attributeExpandSet, permissionService, dataService, languageService);
  }
   ...
   *
   * <p>Example url: /api/v1/person?_method=GET
   *
   * <p>Returns json
   */
  @RequestMapping(
  ...
  public EntityCollection retrieveEntityCollection(
    ...
    if (attributesSet != null) {
      attributesIterable =
          Iterables.filter(
              attributesIterable, attribute -> attributesSet.contains(attribute.toLowerCase()));
    }
   ...
   *
   * <p>Example url: /api/v1/person/99
   */
  @Transactional
   ...
   * <p>Example:
   *
   * <p>Request: {username:admin,password:xxx}
   *
   * <p>Response: {token: b4fd94dc-eae6-4d9a-a1b7-dd4525f2f75d}
   */
  ...
  public LoginResponse login(@Valid @RequestBody LoginRequest login, HttpServletRequest request) {
    ...
    return runAsSystem(
        () -> {
          ...
          String token =
              tokenService.generateAndStoreToken(authentication.getName(), "REST API login");
          return new LoginResponse(
              token, user.getUsername(), user.getFirstName(), user.getLastName());
        });
  }
  ...
  private void updateInternal(
      String entityTypeId, String untypedId, Map<String, Object> entityMap) {
    EntityType meta = dataService.getEntityType(entityTypeId);
    ...
    Entity existing =
        dataService.findOneById(
            entityTypeId, id, new Fetch().field(meta.getIdAttribute().getName()));
    if (existing == null) {
      throw new UnknownEntityException(
          "Entity of type " + entityTypeId + " with id " + id + " not found");
    }
  ...
  private AttributeResponse getAttributePostInternal(
      String entityTypeId,
  ...
  private Object retrieveEntityAttributeInternal(
    ...
    if (attr == null) {
      throw new UnknownAttributeException(
          entityTypeId + " does not have an attribute named " + refAttributeName);
    }
    ...
    String attrHref =
        Href.concatAttributeHref(
            RestController.BASE_URI, meta.getId(), entity.getIdValue(), refAttributeName);
    switch (attr.getDataType()) {
      ...
      case ONE_TO_MANY:
        ...
        for (Entity refEntity : mrefEntities) {
          Map<String, Object> refEntityMap =
              getEntityAsMap(refEntity, attr.getRefEntity(), attributesSet, attributeExpandSet);
          refEntityMaps.add(refEntityMap);
        }

        EntityPager pager =
            new EntityPager(request.getStart(), request.getNum(), (long) count, mrefEntities);
        return new EntityCollectionResponse(
            pager, refEntityMaps, attrHref, null, permissionService, dataService, languageService);
      case CATEGORICAL:
      case XREF:
        Map<String, Object> entityXrefAttributeMap =
            getEntityAsMap(
                (Entity) entity.get(refAttributeName),
                attr.getRefEntity(),
                attributesSet,
                attributeExpandSet);
  ...
  private Map<String, Object> getEntityAsMap(
      Entity entity,
      EntityType meta,
      Set<String> attributesSet,
      Map<String, Set<String>> attributeExpandsSet) {
    ...
    entityMap.put(
        "href", Href.concatEntityHref(RestController.BASE_URI, meta.getId(), entity.getIdValue()));
    ...
    for (Attribute attr : meta.getAtomicAttributes()) {
      ...
      if (attrType == COMPOUND) {
        if (attributeExpandsSet != null
          ...
          entityMap.put(
              ...
              Collections.singletonMap(
                  "href",
                  Href.concatAttributeHref(
                      RestController.BASE_URI, meta.getId(), entity.getIdValue(), attrName)));
        }
        ...
        if (refEntity != null) {
          ...
          Map<String, Object> refEntityMap =
              getEntityAsMap(refEntity, refEntityType, subAttributesSet, null);
        ...
        EntityPager pager =
            new EntityPager(
                0,
                new EntityCollectionRequest().getNum(),
                (long) refEntityMaps.size(),
                mrefEntities);

        EntityCollectionResponse ecr =
  ...
  private Set<String> toAttributeSet(String[] attributes) {
    return attributes != null && attributes.length > 0
        ? Sets.newHashSet(Iterables.transform(Arrays.asList(attributes), String::toLowerCase))
        : null;
  }
