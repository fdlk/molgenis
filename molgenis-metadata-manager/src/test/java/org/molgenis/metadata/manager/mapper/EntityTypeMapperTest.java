...
public class EntityTypeMapperTest {
  ...
  @Mock private EntityTypeParentMapper entityTypeParentMapper;
  ...
  public void testToEntityType() {
    ...
    EditorPackageIdentifier editorPackageIdentifier =
        EditorPackageIdentifier.create("packageId", "package label");
    EditorEntityTypeParent editorEntityTypeParent =
        EditorEntityTypeParent.create(
            "entityTypeParentId",
            "entity type parent label",
            of(EditorAttributeIdentifier.create("parentAttrId0", "parent attribute #0")),
            null);
    List<EditorEntityTypeIdentifier> entityTypeChildren =
        of(EditorEntityTypeIdentifier.create("entityTypeChild0", "entity type #0"));
    List<EditorAttribute> editorAttributes = of(mock(EditorAttribute.class));
    ImmutableList<EditorTagIdentifier> editorTags =
        of(EditorTagIdentifier.create("tag0", "tag #0"));
    EditorAttributeIdentifier idAttribute =
        EditorAttributeIdentifier.create("idAttr", "id attribute");
    EditorAttributeIdentifier labelAttribute =
        EditorAttributeIdentifier.create("labelAttr", "label attribute");
    List<EditorAttributeIdentifier> lookupAttributes =
        of(EditorAttributeIdentifier.create("attr0", "attribute #0"));
    ...
    when(entityTypeParentMapper.toEntityTypeReference(editorEntityTypeParent))
        .thenReturn(parentEntityType);
  ...
  public void testToEditorEntityType() {
    ...
    when(entityType.getString(
            getI18nAttributeName(EntityTypeMetadata.DESCRIPTION, i18nDescriptionLangEn)))
    ...
    when(attributeReferenceMapper.toEditorAttributeIdentifier(idAttribute))
        .thenReturn(editorIdAttribute);
    EditorAttributeIdentifier editorLabelAttribute = mock(EditorAttributeIdentifier.class);
    when(attributeReferenceMapper.toEditorAttributeIdentifier(labelAttribute))
        .thenReturn(editorLabelAttribute);
    @SuppressWarnings("unchecked")
    ...
    when(entityTypeParentMapper.toEditorEntityTypeParent(extendsEntityType))
        .thenReturn(editorEntityTypeParent);
    ...
    EditorEntityType expectedEditorEntityType =
        EditorEntityType.create(
            id,
            label,
            i18nLabel,
