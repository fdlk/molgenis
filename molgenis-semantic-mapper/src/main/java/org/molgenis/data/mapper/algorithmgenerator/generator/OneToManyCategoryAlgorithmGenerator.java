...
public class OneToManyCategoryAlgorithmGenerator extends AbstractCategoryAlgorithmGenerator {
  ...
  public String generate(
    ...
    if (suitableForGeneratingWeightedMap(targetAttribute, sourceAttributes)) {
      stringBuilder
          .append(createAlgorithmNullCheckIfStatement(sourceAttributes))
          .append(createAlgorithmElseBlock(targetAttribute, sourceAttributes));
    } else {
      for (Attribute sourceAttribute : sourceAttributes) {
        stringBuilder.append(
            oneToOneCategoryAlgorithmGenerator.generate(
                targetAttribute,
  ...
  String createAlgorithmElseBlock(Attribute targetAttribute, List<Attribute> sourceAttributes) {
    ...
    if (sourceAttributes.size() > 0) {
      ...
      stringBuilder
          .append("\tSUM_WEIGHT")
          .append(groupCategoryValues(targetAttribute))
          .append("\n}");
    }
    return stringBuilder.toString();
  }
  ...
  boolean suitableForGeneratingWeightedMap(
      Attribute targetAttribute, List<Attribute> sourceAttributes) {
  ...
  public String groupCategoryValues(Attribute attribute) {
    ...
    sortedCategories.sort(
        Comparator.comparingDouble(o -> o.getAmountWrapper().getAmount().getEstimatedValue()));
