...
public class GavinController extends AbstractStaticContentController {
  ...
  public GavinController(
      @Qualifier("gavinExecutors") ExecutorService executorService,
  ...
  /**
   * Shows the gavin page. This page shows the configuration wheels if the annotation resources are
   * not yet properly configured. If the annotation resources are fine, it shows the upload control.
   *
  ...
  public String init(Model model) {
    super.init(model);
    List<String> annotatorsWithMissingResources =
        gavinJobFactory.getAnnotatorsWithMissingResources();
  ...
  public ResponseEntity<String> annotateFile(
      @RequestParam(value = "file") MultipartFile inputFile, @RequestParam String entityTypeId)
      throws IOException {
    ...
    final GavinJobExecution gavinJobExecution =
        gavinJobExecutionFactory.create(idGenerator.generateId(SECURE_RANDOM));
  ...
  /**
   * Retrieves {@link GavinJobExecution} job. May be called by anyone who has the identifier.
   *
  ...
  private String getPageUrl(String jobIdentifier, HttpServletRequest request) {
    ...
    return format(
        "{0}{1}/result/{2}",
        host, menuReaderService.getMenu().findMenuItemPath(GAVIN_APP), jobIdentifier);
  }

  /**
   * Downloads the result of a gavin annotation job.
   *
   * @param response {@link HttpServletResponse} to write the Content-Disposition header to with the
   *     filename
  ...
  public FileSystemResource download(
      HttpServletResponse response, @PathVariable(value = "jobIdentifier") String jobIdentifier)
    ...
    if (!file.exists()) {
      ...
      throw new FileNotFoundException(
          "No result file found for this job. Results are removed every night.");
    }
    response.setHeader(
        "Content-Disposition",
  ...
  private File getDownloadFileForJob(String jobIdentifier) {
    return fileStore.getFile(
        GAVIN_APP + separator + jobIdentifier + separator + "gavin-result.vcf");
  }
  ...
  public Resource downloadErrorReport(
    ...
    if (!file.exists()) {
      ...
      throw new FileNotFoundException(
          "No error report found for this job. Results are removed every night.");
    }
    return new FileSystemResource(file);
  }

  @ExceptionHandler(value = FileNotFoundException.class)
  public void handleFileNotFound(FileNotFoundException ex, HttpServletResponse res)
      throws IOException {
    res.sendError(404, ex.getMessage());
  }

  @ExceptionHandler(value = JobNotFoundException.class)
  public void handleJobNotFound(JobNotFoundException ex, HttpServletResponse res)
      throws IOException {
    res.sendError(404, ex.getMessage());
  }
      ...
      final File[] oldFiles =
          fileStore
              ...
              .listFiles(
                  file ->
                      file.isDirectory()
                          && MILLISECONDS.toSeconds(file.lastModified())
                              < now().minusHours(24).toEpochSecond());
      if (oldFiles != null) {
