...
import static org.molgenis.data.annotation.core.entity.impl.CGDAnnotator.GeneralizedInheritance.DOMINANT;
import static org.molgenis.data.annotation.core.entity.impl.CGDAnnotator.GeneralizedInheritance.DOM_OR_REC;
import static org.molgenis.data.annotation.core.entity.impl.CGDAnnotator.GeneralizedInheritance.OTHER;
import static org.molgenis.data.annotation.core.entity.impl.CGDAnnotator.GeneralizedInheritance.RECESSIVE;
import static org.molgenis.data.annotation.core.entity.impl.CGDAnnotator.GeneralizedInheritance.XLINKED;
...
/**
 * Annotator that can add HGNC_ID and ENTREZ_GENE_ID and other attributes to an EntityType that has
 * a attribute named 'GENE' that must be the HGNC gene dataType.
 *
 * <p>It reads this info from a tab separated CGD file. The location of this file is defined by a
 * RuntimeProperty named 'cgd_location'
 */
@Configuration
public class CGDAnnotator implements AnnotatorConfig {
  ...
  @Autowired private Entity CGDAnnotatorSettings;
  ...
  public enum CGDAttributeName {
    GENE("#GENE", EffectsMetaData.GENE_NAME),
    ...
    AGE_GROUP("AGE GROUP", AGE_GROUP_LABEL),
    INHERITANCE("INHERITANCE", INHERITANCE_LABEL),
  ...
  public void init() {
    ...
    QueryCreator queryCreator =
        new AttributeEqualsQueryCreator(attributeFactory.create().setName(GENE.getAttributeName()));
    ResultFilter resultFilter = new FirstResultFilter();

    EntityAnnotator entityAnnotator =
        new CGDEntityAnnotator(
            CGD_RESOURCE, info, queryCreator, resultFilter, dataService, resources);
  ...
  public Resource cgdResource() {
    return new ResourceImpl(
        CGD_RESOURCE, new SingleResourceConfig(CGD_LOCATION, CGDAnnotatorSettings)) {
      @Override
      public RepositoryFactory getRepositoryFactory() {
        return file ->
            new GeneCsvRepository(
                file,
                GENE.getCgdName(),
  ...
  private List<Attribute> getOutputAttributes() {
    ...
    attributes.add(
        attributeFactory.create().setName(HGNC_ID.getAttributeName()).setDataType(STRING));
    attributes.add(
    ...
    attributes.add(
        attributeFactory
            .create()
            .setName(MANIFESTATION_CATEGORIES.getAttributeName())
            .setDataType(TEXT));
    attributes.add(
        attributeFactory
            .create()
            .setName(INTERVENTION_CATEGORIES.getAttributeName())
            .setDataType(TEXT));
    attributes.add(
        attributeFactory.create().setName(COMMENTS.getAttributeName()).setDataType(TEXT));
    attributes.add(
        attributeFactory
            .create()
            .setName(INTERVENTION_RATIONALE.getAttributeName())
            .setDataType(TEXT));
    attributes.add(
        attributeFactory.create().setName(REFERENCES.getAttributeName()).setDataType(TEXT));

    return attributes;
  }

  private class CGDEntityAnnotator extends AbstractAnnotator {
    public CGDEntityAnnotator(
      ...
      super(
          ...
          new SingleFileLocationCmdLineAnnotatorSettingsConfigurer(
              CGD_LOCATION, CGDAnnotatorSettings));
    }
