...
public class ExacAnnotator implements AnnotatorConfig {
  ...
  @Autowired private Entity exacAnnotatorSettings;
  ...
  public void init() {
    ...
    List<Attribute> resourceMetaData =
        new ArrayList<>(
            asList(
                attributeFactory
                    .create()
                    .setName(EXAC_AF_ResourceAttributeName)
                    .setDataType(STRING),
                attributeFactory
                    .create()
                    .setName(EXAC_AC_HOM_ResourceAttributeName)
                    .setDataType(STRING),
                attributeFactory
                    .create()
                    .setName(EXAC_AC_HET_ResourceAttributeName)
                    .setDataType(STRING)));

    AnnotatorInfo exacInfo =
    ...
    MultiAllelicResultFilter multiAllelicResultFilter =
        new MultiAllelicResultFilter(resourceMetaData, vcfAttributes);
    EntityAnnotator entityAnnotator =
        new AbstractAnnotator(
          ...
          protected Object getResourceAttributeValue(Attribute attr, Entity sourceEntity) {
            String attrName =
                EXAC_AF.equals(attr.getName())
                    ? EXAC_AF_ResourceAttributeName
                    : EXAC_AC_HOM.equals(attr.getName())
                        ? EXAC_AC_HOM_ResourceAttributeName
                        : EXAC_AC_HET.equals(attr.getName())
  ...
  public List<Attribute> createExacOutputAttributes() {
    return newArrayList(
        ...
        getExacAcHomAttr(attributeFactory),
  ...
  Resource exacResource() {
    Resource exacTabixResource =
        new ResourceImpl(
            EXAC_TABIX_RESOURCE, new SingleResourceConfig(EXAC_LOCATION, exacAnnotatorSettings)) {
          @Override
          public RepositoryFactory getRepositoryFactory() {
            return new TabixVcfRepositoryFactory(
                EXAC_TABIX_RESOURCE, vcfAttributes, entityTypeFactory, attributeFactory);
          }
        };
