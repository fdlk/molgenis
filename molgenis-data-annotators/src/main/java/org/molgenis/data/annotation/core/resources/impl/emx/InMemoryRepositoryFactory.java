  ...
  public Repository<Entity> createRepository(File file) throws IOException {
    try {
      ...
      throw new RuntimeException(
          "Unable to create ExcelRepositoryCollection for file:"
              + file.getName()
              + " exception: "
              + e);
    }

    ImmutableMap<String, EntityType> entityMap =
        parser.parse(repositoryCollection, DefaultPackage.PACKAGE_DEFAULT).getEntityMap();
    if (!entityMap.containsKey(id)) {
      throw new RuntimeException(
          "Entity [" + id + "] is not found. Entities found: " + entityMap.keySet());
    }
    ...
    inMemoryRepository.add(
        StreamSupport.stream(
            Spliterators.spliteratorUnknownSize(
                repositoryCollection.getRepository(name).iterator(), Spliterator.ORDERED),
            false));
