...
public class EmailAttributeTypeUpdateIT extends AbstractAttributeTypeUpdateIT {
  ...
  public Object[][] validConversionData() {
    return new Object[][] {
      {"molgenis@test.org", STRING, "molgenis@test.org"},
      {"molgenis@test.org", TEXT, "molgenis@test.org"},
      {"molgenis@test.org", CATEGORICAL, "email label"},
      {"molgenis@test.org", XREF, "email label"}
    };
  }

  /**
   * Valid conversion cases for EMAIL to: STRING, TEXT, CATEGORICAL, XREF
   *
  ...
  /**
   * Invalid conversions cases for EMAIL to: BOOL, INT, LONG, DECIMAL, XREF, CATEGORICAL, SCRIPT,
   * HYPERLINK, HTML, ENUM, DATE, DATE_TIME, MREF, CATEGORICAL_MREF, FILE, COMPOUND
   *
   * @param valueToConvert The value that will be converted
   * @param typeToConvertTo The type to convert to
