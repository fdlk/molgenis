...
public class OneToManyCategoryAlgorithmGeneratorTest extends AbstractMolgenisSpringTest {
  ...
  public void init() {
    ...
    Mockito.when(dataService.findAll(targetRefEntityType.getId()))
        .thenAnswer(
            invocation ->
                Stream.of(
                    targetEntity1, targetEntity2, targetEntity3, targetEntity4, targetEntity5));
    ...
    Entity sourceEntity8 =
        new DynamicEntity(
            targetRefEntityType, of("code", 1, "label", "never/less than 1 per month"));
    Entity sourceEntity9 =
        new DynamicEntity(targetRefEntityType, of("code", 2, "label", "1-3 per month"));
    Entity sourceEntity10 =
        new DynamicEntity(targetRefEntityType, of("code", 3, "label", "once a week"));
    Entity sourceEntity11 =
        new DynamicEntity(targetRefEntityType, of("code", 4, "label", "2-4 per week"));
    Entity sourceEntity12 =
        new DynamicEntity(targetRefEntityType, of("code", 5, "label", "5-6 per week"));
    Entity sourceEntity13 =
        new DynamicEntity(targetRefEntityType, of("code", 6, "label", "once a day"));
    Entity sourceEntity14 =
        new DynamicEntity(targetRefEntityType, of("code", 7, "label", "2-3 per day"));
    Entity sourceEntity15 =
        new DynamicEntity(targetRefEntityType, of("code", 8, "label", "4-5 per day"));
    Entity sourceEntity16 =
        new DynamicEntity(targetRefEntityType, of("code", 9, "label", "6+ per day"));
    ...
    Entity sourceEntity19 =
        new DynamicEntity(targetRefEntityType, of("code", 9, "label", "missing"));

    Mockito.when(dataService.findAll(sourceRefEntityType2.getId()))
        .thenAnswer(invocation -> Stream.of(sourceEntity17, sourceEntity18, sourceEntity19));
  ...
  public void testIsSuitable() {
    Assert.assertTrue(
        categoryAlgorithmGenerator.isSuitable(
            targetAttribute, Arrays.asList(sourceAttribute, sourceAttribute1)));
  }
  ...
  public void testGenerate() {
    ...
    String generateMultipleAttributes =
  ...
  public void testSuitableForGeneratingWeightedMap() {
    Assert.assertTrue(
        categoryAlgorithmGenerator.suitableForGeneratingWeightedMap(
            targetAttribute, Arrays.asList(sourceAttribute, sourceAttribute1)));

    Assert.assertFalse(
        categoryAlgorithmGenerator.suitableForGeneratingWeightedMap(
            targetAttribute, Arrays.asList(sourceAttribute, sourceAttribute1, sourceAttribute2)));
  }
  ...
  public void testHeterogenousGenerator() {
    String expectedAlgorithm =
        "$('MESHED_POTATO_1').map({\"1\":\"4\",\"2\":\"4\",\"3\":\"3\",\"4\":\"2\",\"5\":\"2\",\"6\":\"1\",\"7\":\"1\",\"8\":\"1\",\"9\":\"1\"}, null, null).value();$('MESHED_POTATO').map({\"1\":\"4\",\"2\":\"4\",\"3\":\"4\",\"4\":\"3\",\"5\":\"2\",\"6\":\"2\",\"7\":\"1\"}, null, null).value();$('Stroke').map({\"1\":\"2\",\"2\":\"4\",\"9\":\"9\"}, null, null).value();";
    String actual =
        categoryAlgorithmGenerator.generate(
            targetAttribute,
