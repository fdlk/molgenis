...
public class WikiPathwaysRESTBindingStub implements WikiPathwaysPortType {
  ...
  @Override
  public byte[] getColoredPathway(
      String pwId, String revision, String[] graphId, String[] color, String fileType)
      throws RemoteException {
    try {
      StringBuilder url =
          new StringBuilder(baseUrl + "/getColoredPathway?pwId=" + pwId + "&revision=" + revision);
  ...
  @Override
  public WSCurationTagHistory[] getCurationTagHistory(String pwId, String timestamp)
      throws RemoteException {
    try {
  ...
  @Override
  public boolean removeCurationTag(String pwId, String tagName, WSAuth auth)
      throws RemoteException {
    try {
      String url =
          baseUrl
              + "/removeCurationTag?pwId="
              + pwId
              + "&tagName="
              + tagName
              + "&auth="
      ...
      Document jdomDocument = Utils.connect(url, client);
      String success =
          jdomDocument.getRootElement().getChild("success", WSNamespaces.NS1).getValue();
      if (success.equals("1")) {
        return true;
      }
      return false;
  ...
  @Override
  public boolean saveCurationTag(
      String pwId, String tagName, String tagText, int revision, WSAuth auth)
      throws RemoteException {
    try {
      String url =
          baseUrl
              + "/saveCurationTag?pwId="
              + pwId
              + "&tagName="
              + tagName
              + "&text="
              + URLEncoder.encode(tagText, "UTF-8")
              + "&revision="
              + revision
              + "&auth="
              + auth.getKey()
  ...
  @Override
  public boolean saveOntologyTag(String pwId, String term, String termId, WSAuth auth)
      throws RemoteException {
    try {
      String url =
          baseUrl
              + "/saveOntologyTag?pwId="
              + pwId
              + "&term="
  ...
  @Override
  public boolean updatePathway(
      String pwId, String description, String gpml, int revision, WSAuth auth)
      throws RemoteException {
    try {
