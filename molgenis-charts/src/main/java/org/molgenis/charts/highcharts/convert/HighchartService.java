...
public class HighchartService extends AbstractChartVisualizationService {
  ...
  @Autowired private HighchartSeriesUtil highchartSeriesUtil;

  public HighchartService() {
    super(
        Arrays.asList(
            MolgenisChartType.LINE_CHART,
            MolgenisChartType.SCATTER_CHART,
   ...
   *
   * <p>When the xAxisType equals MolgenisAxisType.DATETIME then the Highcharts Stockchart will be
  ...
  protected Options createBoxPlotChart(BoxPlotChart boxPlotChart) {
    ...
    ChartTitle title =
        new ChartTitle().setText(boxPlotChart.getTitle()).setAlign(ChartAlign.CENTER);
    ...
    options.addSeries(
        highchartSeriesUtil.parseToBoxPlotSeriesList(boxPlotChart.getBoxPlotSeries()));
   ...
   *
   * @param chartConstructorType - When defining the chartConstructorType u can invloed the type of
   *     the Highchart constructor types
   * @return Options
   */
  protected Options createXYDataChart(
      XYDataChart xYDataChart, ChartConstructorType chartConstructorType) {
    Options options = new Options();
    ...
    chart
        .setType(ChartType.getChartType(xYDataChart.getType()))
        .setWidth(xYDataChart.getWidth())
        .setHeight(xYDataChart.getHeight());

    XAxis xAxis = new XAxis();
    xAxis
        .setTitle(new AxisTitle().setText(xYDataChart.getxAxisLabel()).setAlign(AxisAlign.MIDDLE))
        .setType(AxisType.valueOf(xYDataChart.getxAxisType().name()))
        .setOrdinal(false);

    YAxis yAxis = new YAxis();
    yAxis
        .setTitle(new AxisTitle().setText(xYDataChart.getyAxisLabel()).setAlign(AxisAlign.MIDDLE))
