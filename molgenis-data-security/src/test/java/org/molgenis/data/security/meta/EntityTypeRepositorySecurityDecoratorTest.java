...
public class EntityTypeRepositorySecurityDecoratorTest
  ...
  @Captor private ArgumentCaptor<Consumer<List<EntityType>>> consumerCaptor;
  @Mock private EntityType entityType1;
  @Mock private EntityType entityType2;
  @Mock private EntityType entityType3;
  @Mock private EntityType entityType4;
  ...
  public void setUpBeforeMethod() {
    ...
    repo =
        new EntityTypeRepositorySecurityDecorator(
            delegateRepository, systemEntityTypeRegistry, permissionService, dataService);
  }
  ...
  public void countUser() throws Exception {
    ...
    when(delegateRepository.spliterator())
        .thenReturn(asList(entityType0, entityType1).spliterator());
  ...
  public void iteratorUser() throws Exception {
    ...
    EntityType entityType2 =
        when(mock(EntityType.class).getId()).thenReturn(entityType2Name).getMock();
    when(delegateRepository.spliterator())
        .thenReturn(asList(entityType0, entityType1, entityType2).spliterator());
    when(permissionService.hasPermissionOnEntityType(entityType0Name, READ)).thenReturn(true);
    when(permissionService.hasPermissionOnEntityType(entityType1Name, READ)).thenReturn(false);
    when(permissionService.hasPermissionOnEntityType(entityType2Name, READ)).thenReturn(true);
  ...
  public void findAllIdsUser() throws Exception {
    ...
    EntityType entityType2 =
        when(mock(EntityType.class).getId()).thenReturn(entityType2Name).getMock();
    Stream<Object> ids = Stream.of("0", "1");
    when(delegateRepository.findAll(ids))
        .thenReturn(Stream.of(entityType0, entityType1, entityType2));
    when(permissionService.hasPermissionOnEntityType(entityType0Name, READ)).thenReturn(true);
    when(permissionService.hasPermissionOnEntityType(entityType1Name, READ)).thenReturn(false);
    when(permissionService.hasPermissionOnEntityType(entityType2Name, READ)).thenReturn(true);
  ...
  @WithMockUser(
    username = USERNAME,
    roles = {ROLE_WRITEMETA_ENTITY_TYPE}
  )
  @Test
  ...
  @Test(
    ...
    expectedExceptionsMessageRegExp =
        "No \\[WRITEMETA\\] permission on entity type \\[entity\\] with id \\[entity\\]"
  )
  ...
  @Test(
    ...
    expectedExceptionsMessageRegExp = "Deleting system entity meta data \\[entity\\] is not allowed"
  )
  ...
  private void delete() {
    ...
    when(userAuthorityQ.in(
            ROLE,
            newArrayList(
                "ROLE_ENTITY_READ_entity",
                "ROLE_ENTITY_WRITE_entity",
                "ROLE_ENTITY_COUNT_entity",
                "ROLE_ENTITY_NONE_entity",
                "ROLE_ENTITY_WRITEMETA_entity")))
        .thenReturn(userAuthorityQ);
  ...
  @Test(
    expectedExceptions = MolgenisDataException.class,
    expectedExceptionsMessageRegExp = "Updating system entity meta data \\[entity\\] is not allowed"
  )
