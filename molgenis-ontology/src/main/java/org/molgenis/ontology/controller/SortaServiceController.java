...
public class SortaServiceController extends PluginController {
  ...
  public String updateThreshold(
      ...
      @PathVariable String sortaJobExecutionId,
      Model model) {
  ...
  public String matchResult(
      @PathVariable("sortaJobExecutionId") String sortaJobExecutionId, Model model) {
    SortaJobExecution sortaJobExecution = findSortaJobExecution(sortaJobExecutionId);
  ...
  @ResponseBody
  public Map<String, Object> countMatchResult(
      @PathVariable("sortaJobExecutionId") String sortaJobExecutionId) {
    SortaJobExecution sortaJobExecution = findSortaJobExecution(sortaJobExecutionId);
    return ImmutableMap.of(
        "numberOfMatched",
        countMatchedEntities(sortaJobExecution, true),
        "numberOfUnmatched",
  ...
  private void tryDeleteRepository(String entityTypeId) {
    if (dataService.hasRepository(entityTypeId)
        && permissionService.hasPermissionOnEntityType(entityTypeId, Permission.WRITEMETA)) {
  ...
  @ResponseBody
  public EntityCollectionResponse retrieveSortaJobResults(
      @RequestBody SortaServiceRequest sortaServiceRequest, HttpServletRequest httpServletRequest) {
    ...
    QueryRule queryRuleInputEntities =
        new QueryRule(
            Arrays.asList(
                new QueryRule(VALIDATED, EQUALS, isMatched),
                new QueryRule(isMatched ? OR : AND),
    ...
    if (isNotEmpty(filterQuery)) {
      Iterable<String> filteredInputTermIds =
          dataService
              .findAll(
    ...
    findAll.forEach(
        mappingEntity -> {
          ...
          if (matchedTerm != null) {
            outputEntity.put(
                "ontologyTerm",
                SortaServiceUtil.getEntityAsMap(
  ...
  @ResponseBody
  public SortaServiceResponse findMatchingOntologyTerms(
      @RequestBody Map<String, Object> request, HttpServletRequest httpServletRequest) {
    // TODO: less obfuscated request object, let Spring do the matching
    if (request.containsKey("sortaJobExecutionId")
        && !isEmpty(request.get("sortaJobExecutionId").toString())
        && request.containsKey(MatchingTaskContentMetaData.IDENTIFIER)
        && !isEmpty(request.get(MatchingTaskContentMetaData.IDENTIFIER).toString())) {
  ...
  @ResponseBody
  public SortaServiceResponse search(
      @RequestBody Map<String, Object> request, HttpServletRequest httpServletRequest) {
    // TODO: less obfuscated request object, let Spring do the matching
    if (request.containsKey("queryString")
        && !StringUtils.isEmpty(request.get("queryString").toString())
      ...
      return new SortaServiceResponse(
          inputEntity, sortaService.findOntologyTermEntities(ontologyIri, inputEntity));
    }
    return new SortaServiceResponse(
  ...
  public void download(
      @PathVariable String sortaJobExecutionId, HttpServletResponse response, Model model)
      throws IOException {
    CsvWriter csvWriter =
        new CsvWriter(response.getOutputStream(), SortaServiceImpl.DEFAULT_SEPARATOR);
    try {
      SortaJobExecution sortaJobExecution = findSortaJobExecution(sortaJobExecutionId);

      response.setContentType("text/csv");
      response.addHeader(
          "Content-Disposition", "attachment; filename=" + generateCsvFileName("match-result"));
      List<String> columnHeaders = new ArrayList<>();

      EntityType sourceMetaData =
          dataService.getEntityType(sortaJobExecution.getSourceEntityName());
      ...
      dataService
          .findAll(sortaJobExecution.getResultEntityName(), new QueryImpl<>())
  ...
  private String startMatchJob(
    ...
    SortaCsvRepository inputRepository =
        new SortaCsvRepository(
            inputRepositoryName,
            jobName + " input",
            uploadFile,
            entityTypeFactory,
    ...
    SortaJobExecution jobExecution =
        createJobExecution(
            inputRepository, jobName, ontologyIri, SecurityContextHolder.getContext());
  ...
  private List<Entity> getJobsForCurrentUser() {
    ...
    RunAsSystemAspect.runAsSystem(
        () ->
            dataService
                .findAll(SORTA_JOB_EXECUTION, query)
                .forEach(
                    job -> {
  ...
  private long countMatchedEntities(SortaJobExecution sortaJobExecution, boolean isMatched) {
    ...
    return dataService.count(
        sortaJobExecution.getResultEntityName(), new QueryImpl<>(combinedRule));
  }
  ...
  private boolean validateFileHeader(Repository<Entity> repository) {
    boolean containsName =
        StreamSupport.stream(repository.getEntityType().getAttributes().spliterator(), false)
