...
public class CatalogueController extends PluginController {
  ...
  public String showView(
      final @RequestParam(value = "entity", required = false) String selectedEntityName,
      Model model) {
            ...
            entityTypeId -> {
              if (currentUserHasRole(AUTHORITY_SU, AUTHORITY_ENTITY_READ_PREFIX + entityTypeId)) {
                ...
                if (StringUtils.isNotBlank(selectedEntityName)
                    && selectedEntityName.equalsIgnoreCase(entityTypeId)) {
    ...
    if (showEntitySelect) {
      if (StringUtils.isNotBlank(selectedEntityNameValue)) {
        ...
        model.addAttribute(
            "warningMessage", "Entity does not exist or you do not have permission on this entity");
      }
