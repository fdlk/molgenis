...
public class SystemEntityTypePersisterTest extends AbstractMockitoTest {
  ...
  @Mock private AttributeMetadata attrMetaMeta;
  @Mock private MetaDataService metaDataService;

  @Captor private ArgumentCaptor<Stream<Object>> objectIdCaptor;
  ...
  public void removeNonExistingSystemEntities() throws Exception {
    ...
    Attribute refAttr =
        when(mock(Attribute.class).getRefEntity()).thenReturn(refRemovedMeta).getMock();
  ...
  public void persistSystemPackageChange() {
    ...
    when(dataService.findAll(
            eq(ENTITY_TYPE_META_DATA), objectIdCaptor.capture(), eq(EntityType.class)))
        .thenAnswer(invocation -> Stream.empty());
  ...
  public void persistSystemPackageNoChange() {
    ...
    when(dataService.findAll(PACKAGE, Package.class))
        .thenAnswer(invocation -> Stream.of(package0, package1));
    systemEntityTypePersister.persist();
    @SuppressWarnings("unchecked")
    ArgumentCaptor<Stream<Package>> captor = ArgumentCaptor.forClass(Stream.class);
    verify(metaDataService).upsertPackages(captor.capture());
