...
public class VcfImporterServiceTest extends AbstractMockitoTest {
  ...
  public void setUpBeforeMethod() {
    vcfImporterService =
        new VcfImporterService(
            dataService, permissionSystemService, metaDataService, defaultPackage);
  ...
  public void doImportVcfWithoutSamples() {
    ...
    when(outRepo0.add(any(Stream.class)))
        .thenAnswer(
            invocation -> {
  ...
  public void doImportVcfWithSamples() {
    ...
    when(metaDataService.createRepository(argThat(eqName(sampleEntityType0))))
        .thenReturn(outSampleRepo0);
    ...
    Entity entity0 = mock(Entity.class);
    when(entity0.getEntities(VcfAttributes.SAMPLES))
        .thenReturn(Arrays.asList(entity0Sample0, entity0Sample1));
    Entity entity1 = mock(Entity.class);
    when(entity1.getEntities(VcfAttributes.SAMPLES))
        .thenReturn(Arrays.asList(entity1Sample0, entity1Sample1));
    List<Entity> entities = Arrays.asList(entity0, entity1);
    Repository<Entity> repo0 =
        Mockito.spy(
            new AbstractRepository() {
