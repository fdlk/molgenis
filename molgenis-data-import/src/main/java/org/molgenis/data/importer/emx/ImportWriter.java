...
public class ImportWriter {
   ...
   * @param permissionSystemService {@link PermissionSystemService} to give permissions on uploaded
   *     entities
  ...
  public EntityImportReport doImport(EmxImportJob job) {
    // languages first
    importLanguages(
        job.report, job.parsedMetaData.getLanguages(), job.dbAction, job.metaDataChanges);
    runAsSystem(
        () -> {
    ...
    List<EntityType> resolvedEntityTypes =
        entityTypeDependencyResolver.resolve(job.parsedMetaData.getEntities());
  ...
  private void importEntityTypes(
      ImmutableCollection<EntityType> entityTypes, EntityImportReport importReport) {
          ...
          for (EntityType entityType : entities) {
            EntityType existing =
                dataService.findOneById(
                    ENTITY_TYPE_META_DATA, entityType.getId(), EntityType.class);
            if (existing != null) {
  ...
  private void importI18nStrings(
      EntityImportReport report, Map<String, L10nString> i18nStrings, DatabaseAction dbAction) {
  ...
  private void importData(
    ...
    for (final EntityType entityType : resolved) {
      ...
      if (!name.equalsIgnoreCase(LANGUAGE)
          && !name.equalsIgnoreCase(L10N_STRING)
          && dataService.hasRepository(name)) {
        ...
        if ((emxEntityRepo == null)
          ...
          emxEntityRepo =
              source.getRepository(entityType.getId().substring(defaultPackage.length() + 1));
        }
        ...
        if (emxEntityRepo != null) {
          ...
          Iterable<Entity> entities =
              Iterables.transform(emxEntityRepo, emxEntity -> toEntity(entityType, emxEntity));
              ...
              if (emxValue instanceof Iterable<?>) {
                ...
                for (Object emxValueItem : (Iterable<?>) emxValue) {
                  ...
                  if (emxValueItem instanceof Entity) {
                    ...
                    EntityType xrefEntity = attr.getRefEntity();
                    Object entityId =
                        DataConverter.convert(emxValueItem, xrefEntity.getIdAttribute());
  ...
  private void upsertEntityTypes(GroupedEntityTypes groupedEntityTypes) {
    ...
    for (EntityType entityType : updatedEntityTypes) {
      EntityType existing =
          dataService.findOneById(ENTITY_TYPE_META_DATA, entityType.getId(), EntityType.class);
      if (existing != null) {
    ...
    List<EntityType> entityTypes =
        newArrayList(concat(updatedEntityTypes, groupedEntityTypes.getNewEntityTypes()));
    runAsSystem(() -> dataService.getMeta().upsertEntityTypes(entityTypes));
  }

  private static Fetch createEntityTypeWithAttributesFetch() {
    return new Fetch()
        .field(EntityTypeMetadata.PACKAGE)
        .field(
            EntityTypeMetadata.ATTRIBUTES,
            new Fetch().field(AttributeMetadata.ID).field(AttributeMetadata.NAME));
  }

  /**
   * Adds the packages from the packages sheet to the {@link
   * org.molgenis.data.meta.MetaDataService}.
   */
   ...
   */
  private <E extends Entity> int update(
      Repository<E> repo, Iterable<E> entities, DatabaseAction dbAction) {
  ...
  private static <E extends Entity> HugeSet<Object> getExistingEntityIds(
      Repository<E> repo, Iterable<E> entities) {
    String idAttributeName = repo.getEntityType().getIdAttribute().getName();
  ...
  private <E extends Entity> void updateInRepo(
      Repository<E> repo, List<E> existingEntities, List<Integer> existingEntitiesRowIndex) {
    try {
  ...
  private <E extends Entity> void insertIntoRepo(
      Repository<E> repo, List<E> newEntities, List<Integer> newEntitiesRowIndex) {
    try {
      repo.add(newEntities.stream());
    } catch (MolgenisValidationException mve) {
