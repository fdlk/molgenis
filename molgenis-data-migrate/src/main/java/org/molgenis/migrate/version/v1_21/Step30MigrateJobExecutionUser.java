 ...
 *
 * <ul>
 *   <li>Changes JobExecution user columns from foreign key to MolgenisUser to a simple userName.
 * </ul>
 */
public class Step30MigrateJobExecutionUser extends MolgenisUpgrade {
  ...
  private List<String> getJobExecutionEntityNames() {
    return jdbcTemplate.query(
        "SELECT fullName FROM entities WHERE extends = 'JobExecution'",
        (rs, rowNum) -> rs.getString("fullName"));
  }

  /**
   * Checks if this migration step needs to be executed for the current version. There is the
   * possibility a previous version has already done this upgrade because this migration step and
   * its corresponding fix might be added to older versions.
   */
  ...
  private void updateDataType(String entityFullName, String attributeName, String newDataType) {
    ...
    jdbcTemplate.update(
        "UPDATE attributes SET dataType = '"
            + newDataType
            + "', refEntity = NULL WHERE identifier = '"
            + attributeId
            + "'");
  }
  ...
  private void dropForeignKey(String entityFullName) {
    ...
    jdbcTemplate.update(
        "ALTER TABLE `" + entityFullName + "` DROP FOREIGN KEY `" + entityFullName + "_ibfk_1`");
    jdbcTemplate.update("DROP INDEX user ON `" + entityFullName + "`");
    jdbcTemplate.update(
        "UPDATE `"
            + entityFullName
            + "` SET user = (SELECT userName from MolgenisUser WHERE MolgenisUser.ID = `"
            + entityFullName
            + "`.user)");
  }
}
