 ...
 * of the current user - validates permissions when adding, updating or deleting entity types
 *
...
public class EntityTypeRepositorySecurityDecorator extends AbstractRepositoryDecorator<EntityType> {
  ...
  public EntityTypeRepositorySecurityDecorator(
  ...
  public AggregateResult aggregate(AggregateQuery aggregateQuery) {
    if (currentUserIsSuOrSystem()) {
      ...
      throw new MolgenisDataAccessException(
          format("Aggregation on entity [%s] not allowed", getName()));
    }
  }
  ...
  private void deleteEntityPermissions(String entityTypeId) {
    ...
    // User permissions
    List<UserAuthority> userPermissions =
        dataService
            .query(USER_AUTHORITY, UserAuthority.class)
            .in(ROLE, authorities)
            .findAll()
            .collect(toList());
    ...
    // Group permissions
    List<GroupAuthority> groupPermissions =
        dataService
            .query(GROUP_AUTHORITY, GroupAuthority.class)
            .in(ROLE, authorities)
            .findAll()
            .collect(toList());
  ...
  /**
   * Updating entityType meta data is allowed for non-system entities. For system entities updating
   * entityType meta data is only allowed if the meta data defined in Java differs from the meta
   * data stored in the database (in other words the Java code was updated).
   *
  ...
  private EntityType filterReadPermission(EntityType entityType) {
    return entityType != null
        ? filterReadPermission(Stream.of(entityType)).findFirst().orElse(null)
        : null;
  }
  ...
  private Stream<EntityType> filterPermission(
      Stream<EntityType> EntityTypeStream, Permission permission) {
  ...
  private static class FilteredConsumer {
    ...
    void filter(List<EntityType> entityTypes) {
      List<EntityType> filteredEntityTypes =
          entityTypes
              .stream()
              .filter(
                  entityType ->
                      permissionService.hasPermissionOnEntityType(entityType.getId(), READ))
              .collect(toList());
