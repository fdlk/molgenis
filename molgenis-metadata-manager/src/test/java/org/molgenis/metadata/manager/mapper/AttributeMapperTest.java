...
public class AttributeMapperTest {
  ...
  public void testToAttributes() {
    ...
    EditorAttributeIdentifier editorParentAttributeIdentifier =
        EditorAttributeIdentifier.create(parentId, "label");
    ...
    when(entityTypeReferenceMapper.toEntityTypeReference(refEntityTypeId))
        .thenReturn(refEntityType);
    Attribute mappedByAttribute = mock(Attribute.class);
    when(attributeReferenceMapper.toAttributeReference(editorMappedByAttribute))
    ...
    Attribute parentAttr =
        when(mock(Attribute.class).getIdentifier()).thenReturn(parentId).getMock();
    when(attributeFactory.create()).thenReturn(attr, parentAttr);

    EditorEntityType editorEntityType =
        when(mock(EditorEntityType.class).getId()).thenReturn(entityId).getMock();
    EditorAttributeIdentifier editorIdAttributeIdentifier =
    ...
    when(editorEntityType.getLookupAttributes())
        .thenReturn(singletonList(editorIdAttributeIdentifier));
    ...
    EditorAttribute editorAttribute =
        EditorAttribute.create(
            ...
            readonly,
            unique,
            editorTagIdentifiers,
            visibleExpression,
            validationExpression,
            defaultValue,
            sequenceNumber);
    EditorAttribute editorParentAttribute =
        EditorAttribute.create(
            parentId,
            name,
            type,
  ...
  public void testToEditorAttributes() {
    ...
    when(attribute.getString(
            getI18nAttributeName(AttributeMetadata.DESCRIPTION, i18nDescriptionLangEn)))
    ...
    when(attributeReferenceMapper.toEditorAttributeIdentifier(parentAttr))
        .thenReturn(editorParentAttribute);
    EditorEntityTypeIdentifier editorRefEntityType = mock(EditorEntityTypeIdentifier.class);
    when(entityTypeReferenceMapper.toEditorEntityTypeIdentifier(refEntityType))
        .thenReturn(editorRefEntityType);
    ...
    ImmutableList<EditorAttribute> editorAttributes =
        attributeMapper.toEditorAttributes(of(attribute));
    EditorAttribute expectedEditorAttribute =
        EditorAttribute.create(
            id,
            name,
            type,
