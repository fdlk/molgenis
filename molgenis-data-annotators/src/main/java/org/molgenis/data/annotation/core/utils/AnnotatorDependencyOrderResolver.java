...
public class AnnotatorDependencyOrderResolver {
  ...
  public Queue<RepositoryAnnotator> getAnnotatorSelectionDependencyList(
    ...
    for (RepositoryAnnotator annotator : requestedAnnotatorList) {
      if (annotator instanceof EffectBasedAnnotator) {
        ...
        sortedList =
            getSingleAnnotatorDependencyList(
                annotator,
  ...
  private Queue<RepositoryAnnotator> getSingleAnnotatorDependencyList(
      RepositoryAnnotator selectedAnnotator,
      List<RepositoryAnnotator> annotatorList,
      Queue<RepositoryAnnotator> queue,
      EntityType emd,
      EntityTypeFactory entityTypeFactory) {
  ...
  private void resolveAnnotatorDependencies(
      RepositoryAnnotator selectedAnnotator,
      List<RepositoryAnnotator> annotatorList,
      Queue<RepositoryAnnotator> annotatorQueue,
      ...
      selectedAnnotator
          ...
          .filter(
              requiredInputAttribute ->
                  !areRequiredAttributesAvailable(
                      Lists.newArrayList(entityType.getAtomicAttributes()),
  ...
  private void resolveAnnotatorDependencies(
    ...
    if (isRequiredAttributeAvailable(
        annotator.getInfo().getOutputAttributes(), requiredAttribute)) {
      if (areRequiredAttributesAvailable(
          Lists.newArrayList(entityType.getAtomicAttributes()),
  ...
  private boolean areRequiredAttributesAvailable(
      List<Attribute> availableAttributes, List<Attribute> requiredAttributes) {
  ...
  private boolean isRequiredAttributeAvailable(
      List<Attribute> availableAttributes, Attribute requiredAttribute) {
    for (Attribute availableAttribute : availableAttributes) {
      if (requiredAttribute.getName().equals(availableAttribute.getName())) {
        if (requiredAttribute.getDataType() == TEXT) {
          return availableAttribute.getDataType() == TEXT
              || availableAttribute.getDataType() == STRING;
