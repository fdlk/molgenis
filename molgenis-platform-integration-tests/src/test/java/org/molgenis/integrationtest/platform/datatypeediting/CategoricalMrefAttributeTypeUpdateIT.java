...
public class CategoricalMrefAttributeTypeUpdateIT extends AbstractAttributeTypeUpdateIT {
  ...
  public Object[][] validConversionData() {
    ...
    return new Object[][] {
      {entities, MREF, newArrayList("label1", "email label", "hyperlink label")}
    };
  }

  /**
   * Valid conversion cases for CATEGORICAL_MREF to: MREF
   *
  ...
  public void testValidConversion(
      List<Entity> valueToConvert, AttributeType typeToConvertTo, List<String> convertedValue) {
    ...
    entity1
        .getEntities("mainAttribute")
        .forEach(entity -> actualValues.add(entity.getLabelValue().toString()));
  ...
  /**
   * Invalid conversion cases for CATEGORICAL_MREF to: BOOL, STRING, TEXT, SCRIPT INT, LONG,
   * DECIMAL, XREF, CATEGORICAL, EMAIL, HYPERLINK, HTML, ENUM, DATE, DATE_TIME, FILE, COMPOUND,
   * ONE_TO_MANY
   *
   * @param valueToConvert The value that will be converted
   * @param typeToConvertTo The type to convert to
