...
/**
 * Creates AnnotationJob based on its {@link AnnotationJobExecution}. Is a bean so that it can use
 * {@link Autowired} services needed to rehydrate the primitive data types. Runs at execution time.
 */
@Component
public class AnnotationJobFactory {
  private static final Logger LOG = LoggerFactory.getLogger(AnnotationJobFactory.class);

  @Autowired CrudRepositoryAnnotator crudRepositoryAnnotator;
  ...
  public AnnotationJob createJob(AnnotationJobExecution metaData) {
    ...
    List<RepositoryAnnotator> availableAnnotators =
        annotationService
            .getAllAnnotators()
            .stream()
            .filter(RepositoryAnnotator::annotationDataExists)
            .collect(toList());
    List<RepositoryAnnotator> requestedAnnotators =
        Arrays.stream(annotatorNames.split(","))
    ...
    List<RepositoryAnnotator> annotators =
        Lists.newArrayList(
            resolver.getAnnotatorSelectionDependencyList(
                availableAnnotators, requestedAnnotators, repository, entityTypeFactory));
    return new AnnotationJob(
        crudRepositoryAnnotator,
        username,
        annotators,
        repository,
        new ProgressImpl(metaData, jobExecutionUpdater, mailSender),
        runAsAuthentication,
