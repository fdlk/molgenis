...
public class AppRepositoryDecorator extends AbstractRepositoryDecorator<App> {
  ...
  private void activateApp(App app) {
    ...
    if (appSourceArchive != null) {
      ...
      if (fileStoreFile == null) {
        LOG.error(
            "Source archive '{}' for app '{}' missing in file store",
            appSourceArchive.getId(),
            app.getName());
        ...
        if (!app.getUseFreemarkerTemplate()) {
          ...
          if (fileHeader == null) {
            ...
            throw new RuntimeException(
                format(
                    "Missing index.html in %s while option 'Use freemarker template as index.html' was set 'No'",
                    app.getName()));
          }
        }
        //noinspection StringConcatenationMissingWhitespace
        zipFile.extractAll(
            fileStore.getStorageDir()
                + separatorChar
                + FILE_STORE_PLUGIN_APPS_PATH
                + separatorChar
                + app.getId()
                + separatorChar);
  ...
  private void validateResourceZip(App app) {
    ...
    if (appZipMeta != null) {
      ...
      if (fileStoreFile == null) {
        LOG.error(
            "Resource zip '{}' for app '{}' missing in file store",
            appZipMeta.getId(),
            app.getName());
