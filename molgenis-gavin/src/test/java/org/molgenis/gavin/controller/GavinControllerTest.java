...
public class GavinControllerTest extends AbstractMolgenisSpringTest {
  @Autowired private GavinController gavinController;

  @Autowired private ExecutorService executorService;

  @Autowired private GavinJobFactory gavinJobFactory;

  @Autowired private FileStore fileStore;

  @Autowired private UserAccountService userAccountService;

  @Autowired GavinJobExecutionMetaData gavinJobExecutionMetaData;
  ...
  public void testAnnotateFile() throws Exception {
    ...
    doAnswer(
        ...
        .when(gavinJobFactory)
        .createJob(captor.capture());

    when(inputFile.getParentFile()).thenReturn(parentDir);
    when(vcf.getOriginalFilename()).thenReturn(".vcf");

    assertEquals(
        gavinController.annotateFile(vcf, "annotate-file"),
        ResponseEntity.created(URI.create("/plugin/gavin-app/job/ABCDE"))
            .body("/plugin/gavin-app/job/ABCDE"));

    verify(fileStore).createDirectory("gavin-app");
    verify(fileStore).createDirectory("gavin-app" + separator + "ABCDE");
    verify(fileStore)
        .writeToFile(inputStream, "gavin-app" + separator + "ABCDE" + separator + "input.tsv");
  ...
  public void testResult() throws Exception {
    ...
    File resultFile =
        File.createTempFile(
            "gavin", ".vcf", new File(ResourceUtils.getFile(getClass(), "/").getPath()));
    ...
    verify(response)
        .setHeader("Content-Disposition", "inline; filename=\"annotate-file-gavin.vcf\"");
  }
  ...
  public void testCleanUp() throws Exception {
    ...
    when(newJobDir.lastModified())
        .thenReturn(System.currentTimeMillis() - TimeUnit.HOURS.toMillis(23));
    when(newJobDir.isDirectory()).thenReturn(true);
    when(oldJobDir.lastModified())
        .thenReturn(System.currentTimeMillis() - TimeUnit.HOURS.toMillis(24) - 1000);
    ...
    assertTrue(
        fileFilterCaptor.getValue().accept(oldJobDir),
        "cleanUp should remove files that are more than 24 hours old");
    assertFalse(
        fileFilterCaptor.getValue().accept(newJobDir),
