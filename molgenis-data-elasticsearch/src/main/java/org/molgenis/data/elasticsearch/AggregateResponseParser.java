...
class AggregateResponseParser {
  ...
  AggregateResult parseAggregateResponse(
      Attribute aggAttr1,
      Attribute aggAttr2,
    ...
    for (int i = 0; i < nrXLabels; ++i) xLabelsIdx.put(xLabels.get(i), i);
    ...
    if (aggAttr2 != null) {
      ...
      for (int i = 0; i < nrYLabels; ++i) yLabelsIdx.put(yLabels.get(i), i);
    ...
    for (int i = 0; i < nrXLabels; ++i) {
      ...
      for (int j = 0; j < nrYLabels; ++j) yValues.add(0L);
      matrix.add(yValues);
    }
  ...
  private Map<Object, Object> parseAggregations(
      Attribute aggAttr1, Attribute aggAttr2, Attribute aggAttrDistinct, Aggregations aggs) {
  ...
  /**
   * Convert matrix labels that contain ids to label attribute values. Keeps in mind that the last
