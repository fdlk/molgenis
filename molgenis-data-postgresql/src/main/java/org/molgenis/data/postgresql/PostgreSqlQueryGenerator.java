...
class PostgreSqlQueryGenerator {
  ...
  static String getSqlCreateFunctionValidateUpdate(
      EntityType entityType, Collection<Attribute> readonlyTableAttrs) {
    StringBuilder strBuilder =
  ...
  private static boolean isDistinctSelectRequiredRec(
      EntityType entityType, List<QueryRule> queryRules) {
        ...
        if (queryRuleField != null) {
          ...
          if (attr == null) {
            throw new UnknownAttributeException(
                format("Unknown attribute [%s] in entity [%s]", attrName, entityType.getId()));
          }
  ...
  static <E extends Entity> String getSqlSelect(
      EntityType entityType, Query<E> q, List<Object> parameters, boolean includeMrefs) {
    ...
    getPersistedAttributes(entityType)
        .forEach(
            attr -> {
              if (q.getFetch() == null
                ...
                if (isPersistedInOtherTable(attr)) {
                  if (includeMrefs || (attr.getDataType() == ONE_TO_MANY && attr.isMappedBy())) {
                    if (attr.getDataType() == ONE_TO_MANY && attr.isMappedBy()) {
                      ...
                      mrefSelect +=
                          ' '
                              + getSqlSort(attr.getRefEntity(), new QueryImpl<>().sort(orderBy))
                              + ") FROM "
                              + getTableName(attr.getRefEntity())
                              + " WHERE this."
  ...
  static <E extends Entity> String getSqlWhere(
      EntityType entityType, Query<E> q, List<Object> parameters, AtomicInteger mrefFilterIndex) {
      ...
      switch (operator) {
        ...
        case RANGE:
          ...
          if (!(range instanceof Iterable<?>)) {
            throw new MolgenisDataException(
                format(
                    "RANGE value is of type [%s] instead of [Iterable]",
        ...
        case SHOULD:
          ...
          throw new UnsupportedOperationException(
              format(
                  "Query operator [%s] not supported by PostgreSQL repository",
                  operator.toString()));
        default:
  ...
  private static <E extends Entity> String getSqlFrom(EntityType entityType, Query<E> q) {
    ...
    StringBuilder from =
        new StringBuilder(" FROM ").append(getTableName(entityType)).append(" AS this");
  ...
  private static void getJoinQueryAttrsRec(
      EntityType entityType, List<QueryRule> rules, List<Attribute> joinAttrs) {
