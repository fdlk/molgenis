...
public class GavinJob extends TransactionalJob<Void> {
  ...
  private File getFile(String name, String extension) {
    return fileStore.getFile(
        format(
            "{0}{1}{2}{3}{4}.{5}",
            GAVIN_APP, separator, jobIdentifier, separator, name, extension));
  }
  ...
  public Void call(Progress progress) throws Exception {
    ...
    progress.status(
        format(
            "Parsed input file. Found {0} lines ({1} comments, {2} valid VCF, {3} valid CADD, "
                + "{4} errors, {5} indels without CADD score, {6} skipped)",
            lineTypes.size(),
            lineTypes.count(COMMENT),
            lineTypes.count(VCF),
            lineTypes.count(CADD),
            lineTypes.count(ERROR),
            lineTypes.count(INDEL_NOCADD),
