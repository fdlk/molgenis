...
public class StandardsRegistryController extends PluginController {
  ...
  @Autowired
  public StandardsRegistryController(
  ...
  public String init(
      @RequestParam(value = "showPackageNotFound", required = false) String showPackageNotFound,
      Model model) {
  ...
  @ResponseBody
  public PackageSearchResponse search(
      @Valid @RequestBody PackageSearchRequest packageSearchRequest, Model model) {
    String searchQuery = packageSearchRequest.getQuery();
    List<PackageResponse> packageResponses = Lists.newArrayList();

    List<PackageSearchResultItem> searchResults =
        metaDataSearchService.findRootPackages(searchQuery);
    for (PackageSearchResultItem searchResult : searchResults) {
      ...
      List<PackageResponse.Entity> entitiesInPackageFiltered =
          Lists.newArrayList(
              Iterables.filter(
                  entitiesInPackageUnfiltered,
                  entity -> {
      ...
      PackageResponse pr =
          new PackageResponse(
              ...
              p.getLabel(),
              p.getDescription(),
              searchResult.getMatchDescription(),
              entitiesInPackageFiltered,
              getTagsForPackage(p));
    ...
    if (total > 0) {
      if (packageSearchRequest.getOffset() != null) {
        packageResponses =
            packageResponses.subList(packageSearchRequest.getOffset(), packageResponses.size());
      }

      if (packageSearchRequest.getNum() != null
          && packageResponses.size() > packageSearchRequest.getNum()) {
    ...
    int num =
        packageSearchRequest.getNum() != null
            ? packageSearchRequest.getNum()
            : packageResponses.size();

    PackageSearchResponse packageSearchResponse =
        new PackageSearchResponse(searchQuery, offset, num, total, packageResponses);
  ...
  public String showView(
      @RequestParam(value = "package", required = false) String selectedPackageName, Model model) {
  ...
  public PackageResponse getPackage(@RequestParam(value = "package") String packageName) {
    ...
    return new PackageResponse(
        molgenisPackage.getId(),
        molgenisPackage.getLabel(),
        molgenisPackage.getDescription(),
        null,
        getEntitiesInPackage(molgenisPackage.getId()),
  ...
  private List<PackageResponse.Tag> getTagsForPackage(Package p) {
    ...
    for (SemanticTag<Package, LabeledResource, LabeledResource> tag :
        tagService.getTagsForPackage(p)) {
      tags.add(
          new PackageResponse.Tag(
              tag.getObject().getLabel(), tag.getObject().getIri(), tag.getRelation().toString()));
    }
  ...
  private void getEntitiesInPackageRec(
      Package aPackage, List<PackageResponse.Entity> entiesForThisPackage) {
    for (EntityType emd : aPackage.getEntityTypes()) {
      entiesForThisPackage.add(
          new PackageResponse.Entity(emd.getId(), emd.getLabel(), emd.isAbstract()));
    }
  ...
  private static class PackageSearchResponse {
    ...
    public PackageSearchResponse(
        String query, int offset, int num, int total, List<PackageResponse> packages) {
      this.offset = offset;
      this.num = num;
      this.query = query;
      this.total = total;
