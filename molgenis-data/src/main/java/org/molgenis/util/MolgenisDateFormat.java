...
public class MolgenisDateFormat {
  ...
  public static final String FAILED_TO_PARSE_ATTRIBUTE_AS_DATE_MESSAGE =
      "Failed to parse attribute [%s] value [%s] as date. Valid date format is [YYYY-MM-DD].";
  public static final String FAILED_TO_PARSE_ATTRIBUTE_AS_DATETIME_MESSAGE =
      "Failed to parse attribute [%s] value [%s] as datetime. Valid datetime format is [YYYY-MM-DDThh:mm:ssZ]";
  ...
  /**
   * Tries to parse a value representing a LocalDate. Tries many formats, but does require that the
  ...
  /**
   * Tries to parse a value representing an {@link Instant}. Tries many formats, but does require
   * that the date month and year are provided in yyyy-mm-dd form. If too little information is
   * provided, will fill in the start of day and getDefaultZoneId() as default values.
   *
   * @param value the value to parse.
