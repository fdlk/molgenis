...
public class ChartDataServiceImpl implements ChartDataService {
  ...
  @Override
  public XYDataChart getXYDataChart(
      String entityTypeId,
    ...
    final AttributeType attributeXFieldTypeEnum =
        entityType.getAttribute(attributeNameXaxis).getDataType();
    final AttributeType attributeYFieldTypeEnum =
        entityType.getAttribute(attributeNameYaxis).getDataType();
    final List<XYDataSerie> xYDataSeries;

    // Sanity check
    if (AttributeType.DECIMAL.equals(attributeXFieldTypeEnum)
        || AttributeType.INT.equals(attributeXFieldTypeEnum)
        || AttributeType.LONG.equals(attributeXFieldTypeEnum)
        || AttributeType.DATE.equals(attributeXFieldTypeEnum)
        || AttributeType.DATE_TIME.equals(attributeXFieldTypeEnum)
        || AttributeType.DECIMAL.equals(attributeYFieldTypeEnum)
        || AttributeType.INT.equals(attributeYFieldTypeEnum)
        || AttributeType.LONG.equals(attributeYFieldTypeEnum)
        || AttributeType.DATE.equals(attributeYFieldTypeEnum)
        || AttributeType.DATE_TIME.equals(attributeYFieldTypeEnum)) {
        ...
        xYDataSeries =
            Arrays.asList(
                this.getXYDataSerie(
                    ...
                    attributeNameXaxis,
                    attributeNameYaxis,
                    attributeXFieldTypeEnum,
                    attributeYFieldTypeEnum,
                    queryRules));
      } else {
        xYDataSeries =
            this.getXYDataSeries(
                repo,
                entityTypeId,
                attributeNameXaxis,
                attributeNameYaxis,
      ...
      return new XYDataChart(
          xYDataSeries,
          MolgenisAxisType.getType(attributeXFieldTypeEnum),
  ...
  public XYDataSerie getXYDataSerie(
    ...
    serie.setName(
        entityType.getAttribute(attributeNameXaxis).getLabel()
            + " vs "
            + entityType.getAttribute(attributeNameYaxis).getLabel());
   ...
   *
   * <p>with: 1. box plot series as the boxes 2. xy data series as the outliers 3. categories names
   ...
   */
  public BoxPlotChart getBoxPlotChart(
      String entityTypeId,
    ...
    Map<String, List<Double>> boxPlotDataListMap =
        getBoxPlotDataListMap(entityType, iterable, attributeName, split);
   ...
   */
  private Map<String, List<Double>> getBoxPlotDataListMap(
      EntityType entityType, Iterable<Entity> iterable, String attributeName, String split) {
   ...
   * @param sort (Sort)
   */
  private Iterable<Entity> getIterable(
      String entityTypeId, Repository<Entity> repo, List<QueryRule> queryRules, Sort sort) {
  ...
  private Object getJavaValue(
      Entity entity, String attributeName, AttributeType attributeFieldTypeEnum) {
