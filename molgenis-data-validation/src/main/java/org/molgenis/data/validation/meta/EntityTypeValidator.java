...
public class EntityTypeValidator {
  ...
  public EntityTypeValidator(
      DataService dataService, SystemEntityTypeRegistry systemEntityTypeRegistry) {
   ...
   */
  private static void validateOwnLookupAttributes(
              ...
              if (ownAttr == null) {
                throw new MolgenisValidationException(
                    new ConstraintViolation(
                        format(
                            "Lookup attribute [%s] is not part of the entity attributes",
                            ownLookupAttr.getName())));
              }
            });
  }
   ...
   */
  private static void validateOwnLabelAttribute(
      EntityType entityType, Map<String, Attribute> ownAllAttrMap) {
    ...
    if (ownLabelAttr != null) {
      ...
      if (ownAttr == null) {
        throw new MolgenisValidationException(
            new ConstraintViolation(
                format(
                    "Label attribute [%s] is not part of the entity attributes",
                    ownLabelAttr.getName())));
      }
    }
  }
   ...
   */
  private static void validateOwnIdAttribute(
      EntityType entityType, Map<String, Attribute> ownAllAttrMap) {
    ...
    if (ownIdAttr != null) {
      ...
      if (ownAttr == null) {
        throw new MolgenisValidationException(
            new ConstraintViolation(
                format(
                    "Entity [%s] ID attribute [%s] is not part of the entity attributes",
      ...
      if (!AttributeUtils.isIdAttributeTypeAllowed(ownIdAttr)) {
        throw new MolgenisValidationException(
            new ConstraintViolation(
                format(
                    "Entity [%s] ID attribute [%s] type [%s] is not allowed",
                    entityType.getId(), ownIdAttr.getName(), ownIdAttr.getDataType().toString())));
      }
      ...
      if (!ownIdAttr.isUnique()) {
        throw new MolgenisValidationException(
            new ConstraintViolation(
                format(
                    "Entity [%s] ID attribute [%s] is not a unique attribute",
                    entityType.getId(), ownIdAttr.getName())));
      }
      ...
      if (ownIdAttr.isNillable()) {
        throw new MolgenisValidationException(
            new ConstraintViolation(
                format(
                    "Entity [%s] ID attribute [%s] is not a non-nillable attribute",
                    entityType.getId(), ownIdAttr.getName())));
      }
    } else {
  ...
  private static void validateOwnAttributes(EntityType entityType) {
    ...
    if (asStream(entityType.getAllAttributes()).collect(toList()).size() == 0) {
      throw new MolgenisValidationException(
          new ConstraintViolation(
              format(
                  "Entity [%s] does not contain any attributes. Did you use the correct package+entity name combination in both the entities as well as the attributes sheet?",
                  entityType.getId())));
    }
            ...
            attrName -> {
              if (attrMultiMap.get(attrName).size() > 1) {
                throw new MolgenisValidationException(
                    new ConstraintViolation(
                        format(
                            "Entity [%s] contains multiple attributes with name [%s]",
    ...
    if (extendsEntityType != null) {
      Map<String, Attribute> extendsAllAttrMap =
          stream(extendsEntityType.getAllAttributes().spliterator(), false)
              .collect(
                  toMap(
                      Attribute::getName,
                      Function.identity(),
              ...
              attr -> {
                if (extendsAllAttrMap.containsKey(attr.getName())) {
                  throw new MolgenisValidationException(
                      new ConstraintViolation(
                          format(
                              "An attribute with name [%s] already exists in entity [%s] or one of its parents",
  ...
  /**
   * Validates if this entityType extends another entityType. If so, checks whether that parent
  ...
  /**
   * Validates the entity fully qualified name and simple name: - Validates that the entity simple
   * name does not contain illegal characters and validates the name length - Validates that the
   * fully qualified name, simple name and package name are consistent with each other
   *
   * @param entityType entity meta data
  ...
  /**
   * Validates the entity label: - Validates that the label is not an empty string - Validates that
   * the label does not only consist of white space
   *
   * @param entityType entity meta data
  ...
  private static void validateEntityLabel(EntityType entityType) {
    ...
    if (label != null) {
      if (label.isEmpty()) {
        throw new MolgenisValidationException(
            new ConstraintViolation(
                format("Label of EntityType [%s] is empty", entityType.getId())));
