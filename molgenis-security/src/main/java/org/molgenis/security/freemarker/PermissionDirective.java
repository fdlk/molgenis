...
public abstract class PermissionDirective implements TemplateDirectiveModel {
  ...
  public void execute(
    ...
    if (entityTypeId != null) {
      hasPermission =
          permissionService.hasPermissionOnEntityType(entityTypeId, Permission.valueOf(permission));
    }

    if ((plugin != null) && hasPermission) {
      hasPermission =
          permissionService.hasPermissionOnPlugin(plugin, Permission.valueOf(permission));
    }
  ...
  protected abstract void execute(
      boolean hasPermission, Environment env, TemplateDirectiveBody body)
      throws TemplateException, IOException;
}
