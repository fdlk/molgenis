...
public class OwnedEntityRepositoryDecoratorTest {
  ...
  public void query() throws Exception {
    assertEquals(
        ownedEntityRepositoryDecorator.query().getRepository(), ownedEntityRepositoryDecorator);
  }
  ...
  public void deleteStreamEntityExtendsOwned() {
    ...
    Entity myEntity =
        when(mock(Entity.class).getString(OWNER_USERNAME)).thenReturn("username").getMock();
    Entity notMyEntity =
        when(mock(Entity.class).getString(OWNER_USERNAME)).thenReturn("notme").getMock();
  ...
  public void findAllStreamFetchNotExtendsOwned() {
    ...
    Stream<Object> entityIds = Stream.of(id0, id1);
    Fetch decoratedFetch = new Fetch().field(OWNER_USERNAME);
    when(delegateRepository.findAll(entityIds, decoratedFetch))
  ...
  public void consumeBatchedExtendsOwned() {
    ...
    verify(delegateRepository)
        .forEachBatched(eq(decoratedFetch), consumerCaptor.capture(), eq(123));
