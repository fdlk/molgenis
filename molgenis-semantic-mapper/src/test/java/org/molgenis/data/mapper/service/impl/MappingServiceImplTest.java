...
public class MappingServiceImplTest extends AbstractMolgenisSpringTest {
  ...
  public void beforeMethod() {
    ...
    when(metaDataService.createRepository(
            argThat(obj -> obj != null && obj.getId().equals(hopMetaData.getId()))))
        .thenReturn(hopRepo);
    when(metaDataService.createRepository(
  ...
  public void testAddMappingProject() {
    ...
    MappingProject mappingProject =
        mappingService.addMappingProject(projectName, user, hopMetaData.getId());
  ...
  public void testAddExistingTarget() {
    MappingProject mappingProject =
        mappingService.addMappingProject("Test123", user, hopMetaData.getId());
  ...
  public void createMetaWithNullParameters() {
    ...
    EntityType targetMetadata =
        mappingService.createTargetMetadata(mappingTarget, "target id", null, null, null);
  ...
  public void testApplyMappingsAdd() {
    ...
    doAnswer(
        ...
        .when(geneRepo)
        .forEachBatched(any(Consumer.class), eq(MAPPING_BATCH_SIZE));
  ...
  @Test(
    ...
    expectedExceptionsMessageRegExp =
        "Target repository does not contain the following attribute: COUNTRY_1"
  )
  ...
  private MappingProject createMappingProjectWithMappings() {
    MappingProject mappingProject =
        mappingService.addMappingProject("TestRun", user, hopMetaData.getId());
