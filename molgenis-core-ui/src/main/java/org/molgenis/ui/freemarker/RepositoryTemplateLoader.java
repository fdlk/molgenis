...
public class RepositoryTemplateLoader implements TemplateLoader {
  ...
  public Object findTemplateSource(String name) throws IOException {
    FreemarkerTemplate template =
        dataService.findOne(
            FREEMARKER_TEMPLATE,
            new QueryImpl<FreemarkerTemplate>().eq("Name", name),
  ...
  /**
   * Template source class. Needed to have a correct {@link #equals(Object)} method. Also used for
  ...
  public class TemplateSource {
    ...
    public String toString() {
      return String.format(
          "Freemarker Template \"%s\" (loaded on %TT.%2$TL)", template.getName(), lastModified);
    }
