...
public class VcfToEntity {
  ...
  private EntityType createSampleEntityType(
      String entityTypeId, Iterable<VcfMetaFormat> formatMetaData) {
    ...
    if (formatMetaData.iterator().hasNext()) {
      ...
      Attribute idAttr =
          attrMetaFactory.create().setName(ID).setAggregatable(true).setVisible(false);
      Attribute nameAttr =
          attrMetaFactory
              .create()
              .setName(NAME)
              ...
              .setAggregatable(true)
              .setNillable(false);
      Attribute originalNameAttr =
          attrMetaFactory.create().setName(ORIGINAL_NAME).setDataType(TEXT);
      ...
      for (VcfMetaFormat meta : formatMetaData) {
        ...
        if (NameValidator.KEYWORDS.contains(name)
            || NameValidator.KEYWORDS.contains(name.toUpperCase())) {
          name = name + "_";
        }
        Attribute attr =
            attrMetaFactory
  ...
  private static AttributeType vcfReaderFormatToMolgenisType(VcfMetaInfo vcfMetaInfo) {
    ...
    try {
      ...
      throw new GenotypeDataException(
          "Error parsing length of vcf info field. "
              + number
              + " is not a valid int or expected preset (A, R, G, .)",
          ex);
    }
    switch (vcfMetaInfo.getType()) {
      ...
      default:
        throw new MolgenisDataException(
            format("Unknown vcf info type [%s]", vcfMetaInfo.getType()));
    }
  }
  ...
  public Entity toEntity(VcfRecord vcfRecord) {
    ...
    entity.set(
        ALT,
        StringUtils.join(Lists.transform(vcfRecord.getAlternateAlleles(), Allele::toString), ','));
    ...
    if (sampleEntityType != null) {
      List<Entity> samples =
          createSampleEntities(vcfRecord, entity.get(POS) + "_" + entity.get(ALT), id);
  ...
  private List<Entity> createSampleEntities(
      VcfRecord vcfRecord, String entityPosAlt, String entityId) {
        ...
        for (int i = 0; i < format.length; i = i + 1) {
          ...
          if (attr != null) {
            ...
            if (Arrays.equals(EMPTY_FORMAT, format)) {
              ...
              throw new MolgenisDataException(
                  "Sample entity contains an attribute ["
                      + format[i]
      ...
      if (vcfInfoFlagFieldKeys.contains(vcfInfo.getKey())) {
        ...
        if (vcfInfoVal == null) {
          ...
          if (Float.isNaN((Float) vcfInfoVal)) {
            ...
            val =
                new BigDecimal(String.valueOf(vcfInfoVal))
  ...
  private String toAttributeName(String vcfInfoFieldKey) {
    ...
    if (attrName == null) {
      throw new RuntimeException(
          format("Missing attribute for VCF info field [%s]", vcfInfoFieldKey));
    }
    return attrName;
  }
