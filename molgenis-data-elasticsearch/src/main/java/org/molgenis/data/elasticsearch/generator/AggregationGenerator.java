...
class AggregationGenerator {
   ...
   * The precision_threshold options allows to trade memory for accuracy, and defines a unique count
   * below which counts are expected to be close to accurate. Above this value, counts might become
   * a bit more fuzzy. The maximum supported value is 40000, thresholds above this number will have
   * the same effect as a threshold of 40000.
   */
  ...
  List<AggregationBuilder> createAggregations(
    ...
    if (aggAttr1 == null) {
      throw new IllegalArgumentException(
          "Aggregation requires at least one isAggregatable attribute");
    }
    if (!aggAttr1.isAggregatable()) {
      throw new IllegalArgumentException(
          "Attribute is not isAggregatable [ " + aggAttr1.getName() + "]");
    }
    if (aggAttr2 != null && !aggAttr2.isAggregatable()) {
      throw new IllegalArgumentException(
          "Attribute is not isAggregatable [ " + aggAttr2.getName() + "]");
    }
  ...
  private List<AggregationBuilder> createAggregations(
      LinkedList<Attribute> attrs, Attribute parentAttr, Attribute distinctAttr) {
    ...
    AggregationBuilder termsAgg =
        AggregationBuilders.terms(termsAggName).size(MAX_VALUE).field(termsAggFieldName);
    aggs.add(termsAgg);
    ...
    if (attr.isNillable()) {
      ...
      AggregationBuilder missingTermsAgg =
          AggregationBuilders.missing(missingAggName).field(missingAggFieldName);
    ...
    if (attrs.isEmpty() && distinctAttr != null) {
      String cardinalityAggName =
          distinctAttr.getName() + FieldConstants.AGGREGATION_DISTINCT_POSTFIX;
      String cardinalityAggFieldName = getAggregateFieldName(distinctAttr);
      CardinalityAggregationBuilder distinctAgg =
          AggregationBuilders.cardinality(cardinalityAggName)
      ...
      if (AggregateUtils.isNestedType(distinctAttr)) {
        ...
        NestedAggregationBuilder nestedBuilder =
            AggregationBuilders.nested(nestedAggName, nestedAggFieldName);
        ...
        if (AggregateUtils.isNestedType(attr)) {
          ...
          ReverseNestedAggregationBuilder reverseNestedBuilder =
              AggregationBuilders.reverseNested(reverseAggName);
    ...
    if (parentAttr != null && AggregateUtils.isNestedType(parentAttr)) {
      String reverseAggName =
          parentAttr.getName() + AggregationGenerator.AGGREGATION_REVERSE_POSTFIX;
      ReverseNestedAggregationBuilder reverseNestedAgg =
          AggregationBuilders.reverseNested(reverseAggName);
      for (AggregationBuilder agg : aggs) {
