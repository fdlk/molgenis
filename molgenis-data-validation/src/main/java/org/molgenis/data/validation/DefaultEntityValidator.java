...
public class DefaultEntityValidator implements EntityValidator {
  ...
  public void validate(
      Iterable<? extends Entity> entities, EntityType meta, DatabaseAction dbAction)
  ...
  private Set<ConstraintViolation> checkNotNull(
      Iterable<? extends Entity> entities, EntityType meta) {
      ...
      if (!attr.isNillable() && !attr.equals(meta.getIdAttribute()) && !attr.isAuto()) {
        ...
        for (Entity entity : entities) {
          ...
          if (mustDoNotNullCheck(meta, attr, entity) && entity.get(attr.getName()) == null) {
            String message =
                String.format(
                    "The attribute '%s' of entity '%s' with key '%s' can not be null.",
                    attr.getName(),
                    meta.getId(),
  ...
  public boolean mustDoNotNullCheck(EntityType entityType, Attribute attr, Entity entity) {
    ...
    if (StringUtils.isNotBlank(attr.getVisibleExpression())
        && !expressionValidator.resolveBooleanExpression(attr.getVisibleExpression(), entity))
      return false;

    return true;
  }

  private Set<ConstraintViolation> checkUniques(
      Iterable<? extends Entity> entities, EntityType meta, DatabaseAction dbAction) {
    Set<ConstraintViolation> violations = Sets.newLinkedHashSet();

    for (Attribute attr : meta.getAtomicAttributes()) {
      if (attr.isUnique()
          && !attr.equals(meta.getIdAttribute())
          && !(attr.equals(meta.getLabelAttribute())
        ...
        if (!values.isEmpty()) {
          ...
          long count =
              dataService.count(entityTypeId, new QueryImpl<>().in(attr.getName(), values));
          if (count > 0) {
            ...
            while (it.hasNext() && (found < count)) {
              ...
              Object value = entity.get(attr.getName());
              Entity existing =
                  dataService.findOne(entityTypeId, new QueryImpl<>().eq(attr.getName(), value));
