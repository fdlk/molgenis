...
/**
 * Get the Molgenis version from molgenis-server.properties or, in absence there, from a {@link
 * DataSource}.
 *
 * <p>The current Molgenis version is found in the server properties under key <code>
 * molgenis.version</code>.
 *
 * <p>
 *
 * <p>If this key is not present, we're either looking at a molgenis 1.4.3 or a new install should
 * be run. We'll check the datasource for the presence of a mysql entities table. If no <code>
 * MolgenisUser</code> table exists in the datasource's database, a new install is assumed, so the
 * version will be set to the current version and no upgrade will take place.
 *
 * <p>
 *
 * <p>This is done so we can upgrade the database. If we store it in the database we must access the
 * database to get it but we must upgrade before we can access the database...
 */
@Service
public class MolgenisVersionService {
  ...
  public MolgenisVersionService(DataSource dataSource) {
    if (MigrationUtils.getVersion() == null) {
      ...
      if (isPopulatedDatabase(dataSource)) {
        LOG.info(
            "Database is populated. Setting molgenis-server.properties to 0. (Molgenis 1.4.3)");
        updateToVersion(0);
      } else {
        LOG.info(
            "Database is empty. Setting molgenis-server.properties to current version. (Clean install)");
    ...
    try {
      return (boolean)
          JdbcUtils.extractDatabaseMetaData(
              ...
              dbmd -> {
                ResultSet tables =
                    dbmd.getTables(null, null, "MolgenisUser", new String[] {"TABLE"});
  ...
  public void updateToVersion(int version) {
    ...
    try (OutputStream out =
        new FileOutputStream(MigrationUtils.getMolgenisServerPropertiesFile())) {
