...
public abstract class SystemEntityType extends EntityType {
  ...
  /**
   * Construct entity meta data for an entity with the given name stored in a system package with
  ...
  public SystemEntityType(String entityName, String systemPackageName) {
    if (!systemPackageName.startsWith(PACKAGE_SYSTEM)) {
      throw new IllegalArgumentException(
          format(
              "Entity [%s] must be located in package [%s] instead of [%s]",
              entityName, PACKAGE_SYSTEM, systemPackageName));
    }
   ...
   *
   * @return Set containing the {@link SystemEntityType}s that this {@link SystemEntityType}'s
