...
public class SnpEffRunner {
          ...
          if (effects.isEmpty()) {
            ...
            if (chromosome != null && position != null) {
              ...
              if (snpEffEntity != null) {
                effects.addAll(
                    getSnpEffectsFromSnpEffEntity(
                        sourceEntity, snpEffEntity, getTargetEntityType(sourceEMD)));
              } else {
  ...
  /**
   * Returns the next entity containing SnpEff annotations if its Chrom and Pos match. This
   * implementation works because SnpEff always returns output in the same order as the input
   *
   ...
   * @return {@link Entity}
   */
  private Entity getSnpEffEntity(
      PeekingIterator<Entity> snpEffResultIterator, String chromosome, int position) {
    if (snpEffResultIterator.hasNext()) {
      Entity entityCandidate = snpEffResultIterator.peek();
      if (chromosome.equals(entityCandidate.getString(VcfAttributes.CHROM))
          && position == entityCandidate.getInt(VcfAttributes.POS)
          && entityCandidate.getString(SnpEffRunner.ANN) != null) {
  ...
  private List<Entity> getSnpEffectsFromSnpEffEntity(
      Entity sourceEntity, Entity snpEffEntity, EntityType effectsEMD) {
    ...
    for (String annotation : annotations) {
      ...
      if (fields.length >= 15) {
        ...
        LOG.info(
            "No results for CHROM:{} POS:{} REF:{} ALT:{} ",
            effect.getString(VcfAttributes.CHROM),
            effect.getString(VcfAttributes.POS),
            effect.getString(VcfAttributes.REF),
  ...
  public File getInputVcfFile(Iterator<Entity> source) throws IOException {
    ...
    try (BufferedWriter bw =
        new BufferedWriter(new OutputStreamWriter(new FileOutputStream(vcf), CHARSET))) {
      bw.write(
          VcfAttributes.CHROM
              + "\t"
              + VcfAttributes.POS
  ...
  /**
   * Gets the path to the SnpEff JAR. Returns null when the path is not found or
   * snpEffAnnotatorSettings is null.
   *
    ...
    if (snpEffAnnotatorSettings != null) {
      snpEffPath =
          RunAsSystemAspect.runAsSystem(
              () ->
                  snpEffAnnotatorSettings.getString(
                      SnpEffAnnotatorSettings.Meta.SNPEFF_JAR_LOCATION));
