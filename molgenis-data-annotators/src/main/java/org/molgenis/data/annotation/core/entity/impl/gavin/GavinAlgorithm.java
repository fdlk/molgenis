...
public class GavinAlgorithm {
  ...
  /**
   * Sensitivity is more important than specificity, so we can adjust this parameter to globally
   * adjust the thresholds.
   *
   * <ul>
   *   <li>At a setting of 1, ie. default behaviour, GAVIN is 89% sensitive and 83% specific
   *   <li>at a setting of 5, a more sensitive setting, GAVIN is 92% sensitive and 78% specific
   * </ul>
   *
   * Technically we lose more than we gain, but having >90% sensitivity is really important. Better
   * to have a few more false positives than to miss a true positive.
   */
   ...
   */
  public Judgment classifyVariant(
      Impact impact,
      Double caddScaled,
      Double exacMAF,
      String gene,
          ...
          if (gavinThresholds.isAboveSpec95thPerCADDThreshold(caddScaled)) {
            return Judgment.create(
                ...
                "Variant CADD score of "
                    + caddScaled
                    + " is greater than "
                    + gavinThresholds.getSpec95thPerCADDThreshold()
                    + " for this gene.");
          }
          if (gavinThresholds.isBelowSens95PerCADDThreshold(caddScaled)) {
            return Judgment.create(
                ...
                "Variant CADD score of "
                    + caddScaled
                    + " is less than "
                    + gavinThresholds.getSens95thPerCADDThreshold()
                    + " for this gene.");
          }
          //this rule does not classify apparently, just continue onto the next rules
          break;
    ...
    if (gavinThresholds.isAbovePathoMAFThreshold(exacMAF)) {
      ...
      return Judgment.create(
          ...
          "Variant MAF of "
              + exacMAF
              + " is greater than "
              + gavinThresholds.getPathoMAFThreshold()
              + ".");
    }
    ...
    if (impact != null) {
      ...
      if (gavinThresholds.getCategory() == I3
   ...
   * @return the {@link Judgment} of the gavin algorithm
   */
  public Judgment genomewideClassifyVariant(
      Impact impact, Double caddScaled, Double exacMAF, String gene) {
    ...
    if (exacMAF > GENOMEWIDE_MAF_THRESHOLD) {
      return Judgment.create(
          ...
          "Variant MAF of "
              + exacMAF
              + " is not rare enough to generally be considered pathogenic.");
    }
    if (impact == MODIFIER) {
      return Judgment.create(
          Benign,
    ...
    if (caddScaled != null && caddScaled > GENOMEWIDE_CADD_THRESHOLD) {
      return Judgment.create(
          ...
          "Variant MAF of "
              + exacMAF
              + " is rare enough to be potentially pathogenic and its CADD score of "
              + caddScaled
              + " is greater than a global threshold of "
    ...
    if (caddScaled != null && caddScaled <= GENOMEWIDE_CADD_THRESHOLD) {
      return Judgment.create(
          ...
          "Variant CADD score of "
              + caddScaled
              + " is less than a global threshold of "
              + GENOMEWIDE_CADD_THRESHOLD
              + ", although the variant MAF of "
              + exacMAF
    ...
    return Judgment.create(
        ...
        genomewide,
        gene,
        "Unable to classify variant as benign or pathogenic. The combination of "
            + impact
            + " impact, a CADD score of "
            + caddScaled
            + " and MAF of "
            + exacMAF
            + " in "
