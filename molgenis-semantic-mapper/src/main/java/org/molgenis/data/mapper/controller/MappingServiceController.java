...
public class MappingServiceController extends PluginController {
  ...
  @Autowired private MappingJobExecutionFactory mappingJobExecutionFactory;

  @Autowired private UserAccountService userAccountService;

  @Autowired private JobExecutor jobExecutor;

  @Autowired private JobsController jobsController;
  ...
  public String addMappingProject(
    ...
    MappingProject newMappingProject =
  ...
  public String removeAttributeMapping(
    ...
    if (hasWritePermission(project)) {
      project
          .getMappingTarget(target)
          .getMappingForSource(source)
          .deleteAttributeMapping(attribute);
  ...
  public List<ExplainedAttribute> getSemanticSearchAttributeMapping(
    ...
    Map<Attribute, ExplainedAttribute> relevantAttributes =
        semanticSearchService.decisionTreeToFindRelevantAttributes(
  ...
  public String attributeMappingFeedback(
      ...
      if (!sourceAttributeNames.isEmpty()) {
        List<Attribute> sourceAttributes =
            sourceAttributeNames
                ...
                .map(
                    attributeName ->
                        entityMapping.getSourceEntityType().getAttribute(attributeName))
                .collect(Collectors.toList());
    ...
    FluentIterable<Entity> sourceEntities =
        FluentIterable.from(() -> dataService.findAll(source).iterator()).limit(10);
  ...
  private void autoGenerateAlgorithms(
    ...
    attributes.forEach(
        attribute ->
            algorithmService.autoGenerateAlgorithm(
                sourceEntityType, targetEntityType, mapping, attribute));
    mappingService.updateMappingProject(project);
  }
  ...
  private boolean hasWritePermission(MappingProject project, boolean logInfractions) {
    boolean result =
        currentUserIsSu() || project.getOwner().getUsername().equals(getCurrentUsername());

    if (logInfractions && !result) {
      LOG.warn(
          "User "
              + getCurrentUsername()
              + " illegally tried to modify mapping project with id "
