...
public class ChartController {
  ...
  public Options renderXYDataChart(@Valid @RequestBody XYDataChartRequest request, Model model) {
    ...
    XYDataChart xYDataChart =
        chartDataService.getXYDataChart(
            request.getEntity(),
            request.getX(),
            request.getY(),
            request.getSplit(),
            query != null ? query.getRules() : Collections.emptyList());
    ...
    ChartVisualizationService service =
        chartVisualizationServiceFactory.getVisualizationService(
  ...
  public Options renderPlotBoxChart(@Valid @RequestBody BoxPlotChartRequest request, Model model) {
    Query<Entity> query = request.getQuery();
    BoxPlotChart chart =
        chartDataService.getBoxPlotChart(
            request.getEntity(),
    ...
    ChartVisualizationService service =
        chartVisualizationServiceFactory.getVisualizationService(MolgenisChartType.BOXPLOT_CHART);
   ...
   *
   * <p>User can only view his own files he created with the charts module
   */
   ...
   *
   * <p>Returns a piece of javascript that can be retrieved by an html page with an ajax request.
   *
   * <p>The page must have an element with id named 'container'. The svg image will be added to this
   * container element.
   */
  @RequestMapping(
    value = "/heatmap",
  ...
  public String renderHeatMap(@Valid @RequestBody HeatMapRequest request, Model model)
    ...
    DataMatrix matrix =
        chartDataService.getDataMatrix(
            request.getEntity(), request.getX(), request.getY(), request.getQueryRules());
    ...
    ChartVisualizationService service =
        chartVisualizationServiceFactory.getVisualizationService(MolgenisChartType.HEAT_MAP);
