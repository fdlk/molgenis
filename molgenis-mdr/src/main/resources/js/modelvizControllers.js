...
var Visualiser = function($scope, $element, api) {
	...
	paper.on('cell:pointerclick', function(cellView, evt, x, y) {
		...
		if ( cellView.model._ENTITY !== undefined) {
			$scope.$parent.name = cellView.model._ENTITY.name;
			$scope.$parent.description = cellView.model._ENTITY.description;			
		}
		debug = cellView
	});
...
//		});
	...
	self.doLayout = function() {
		...
		if (self.$element[0].offsetWidth > dim.width) {
			paper.fitToContent(self.$element[0].offsetWidth +50 , dim.height, 10);
		} else {
			paper.fitToContent(dim.width + 50, dim.height, 10);
		}
		$scope.diagramstyle = {
			'width' : dim.width + 70,
...
Visualiser.prototype.getNewData = function(entityId, levels,$scope) {
	...
	self.api.getEntity(entityId, levels).success(function(response) {
		...
		$scope.$parent.name = response.name;
		$scope.$parent.description = response.description;			
		debug = response;
	});
}
