...
/**
 * Persists {@link SystemEntityType} in the meta data {@link
...
public class SystemEntityTypePersister {
  ...
  @Autowired
  public SystemEntityTypePersister(
  ...
  private void persistMetadataMetadata() {
    ...
    List<EntityType> metaEntityTypeList =
        systemEntityTypeRegistry
            .getSystemEntityTypes()
            .filter(MetaDataService::isMetaEntityType)
            .collect(toList());
    List<EntityType> resolvedEntityTypeList =
        entityTypeDependencyResolver.resolve(metaEntityTypeList);

    resolvedEntityTypeList.forEach(
        metaEntityType -> {
  ...
  private void injectExistingPackageIdentifiers(List<Package> systemPackages) {
    Map<String, Package> existingPackageMap =
        dataService.findAll(PACKAGE, Package.class).collect(toMap(Package::getId, pack -> pack));
