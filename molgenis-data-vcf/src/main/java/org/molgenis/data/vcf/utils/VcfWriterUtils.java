...
public class VcfWriterUtils {
  ...
  private static final LinkedList<String> VCF_ATTRIBUTE_NAMES =
      new LinkedList<>(Arrays.asList(CHROM, POS, ID, REF, ALT, QUAL, FILTER));
   ...
   */
  public static void writeVcfHeader(
      File inputVcfFile, BufferedWriter outputVCFWriter, List<Attribute> addedAttributes)
   ...
   *
   * @param attributesToInclude , the Attribute to write to the VCF file, if empty writes all
   *     attributes
   */
  public static void writeVcfHeader(
      File inputVcfFile,
  ...
  public static void writeToVcf(Entity vcfEntity, BufferedWriter writer)
  ...
  private static void checkColumnHeaders(
      BufferedWriter outputVCFWriter, Scanner inputVcfFileScanner, String line)
  ...
  private static void writeColumnHeaders(BufferedWriter outputVCFWriter, String line)
      throws IOException {
    outputVCFWriter.write(line);
    outputVCFWriter.newLine();
  }

  private static void writeInfoHeaders(
      BufferedWriter outputVCFWriter,
      List<Attribute> annotatorAttributes,
      List<String> attributesToInclude,
      Map<String, String> infoHeaderLinesMap)
      throws IOException {
    Map<String, Attribute> annotatorAttributesMap =
        VcfUtils.getAttributesMapFromList(annotatorAttributes);
    writeExistingInfoHeaders(outputVCFWriter, infoHeaderLinesMap, annotatorAttributesMap);
    writeAddedInfoHeaders(
        outputVCFWriter, attributesToInclude, annotatorAttributesMap, infoHeaderLinesMap);
  }

  private static void writeAddedInfoHeaders(
      BufferedWriter outputVCFWriter,
      List<String> attributesToInclude,
    ...
    for (Attribute annotatorInfoAttr : annotatorAttributes.values()) {
      if (attributesToInclude.isEmpty()
        ...
        outputVCFWriter.write(
            createInfoStringFromAttribute(
                annotatorAttributes.get(annotatorInfoAttr.getName()),
                attributesToInclude,
                infoHeaderLinesMap.get(annotatorInfoAttr.getName())));
        outputVCFWriter.newLine();
      }
    }
  }

  private static String createInfoStringFromAttribute(
      Attribute infoAttribute, List<String> attributesToInclude, String currentInfoField) {
      ...
      if (isReferenceType(infoAttribute) && !attributeName.equals(SAMPLES)) {
        String currentAttributesString =
            currentInfoField != null
                ? currentInfoField.substring(
                    (currentInfoField.indexOf("'") + 1), currentInfoField.lastIndexOf("'"))
                : "";
        writeRefAttributePartsToInfoDescription(
            infoAttribute, attributesToInclude, attributeName, sb, currentAttributesString);
      } else {
  ...
  private static void writeRefAttributePartsToInfoDescription(
    ...
    sb.append(
        refAttributesToString(atomicAttributes, attributesToInclude)
            .replace("\\", "\\\\")
            .replace("\"", "\\\"")
            .replace("\n", " "));
    sb.append("'");
  ...
  private static void addStandardFieldsToVcf(Entity vcfEntity, BufferedWriter writer)
      throws IOException {
  ...
  private static void writeInfoData(
      Entity vcfEntity,
      BufferedWriter writer,
      List<Attribute> annotatorAttributes,
      List<String> attributesToInclude)
    ...
    writer.append(
        infoFieldStrs.stream().collect(joining(String.valueOf(ANNOTATION_FIELD_SEPARATOR))));

    String refEntityAttributesInfoFields =
        parseRefAttributesToDataString(vcfEntity, annotatorAttributes, attributesToInclude);
  ...
  private static String parseRefAttributesToDataString(
      Entity vcfEntity, List<Attribute> annotatorAttributes, List<String> attributesToInclude) {
    ...
    if (refEntityInfoFields.length() > 0
        && refEntityInfoFields.charAt(refEntityInfoFields.length() - 1)
  ...
  private static void parseRefFieldsToInfoField(
      Iterable<Entity> refEntities,
      Attribute attribute,
      StringBuilder refEntityInfoFields,
      List<Attribute> annotatorAttributes,
      List<String> attributesToInclude) {
    ...
    for (Entity refEntity : refEntities) {
      ...
      if (!secondValuePresent) {
        ...
        addEntityValuesToRefEntityInfoField(
            refEntityInfoFields,
            refEntity,
            refAttributes,
            annotatorAttributes,
  ...
  private static void addEntityValuesToRefEntityInfoField(
      StringBuilder refEntityInfoFields,
      Entity refEntity,
      Iterable<Attribute> refAttributes,
      List<Attribute> annotatorAttributes,
      List<String> attributesToInclude) {
    boolean previousValuePresent = false;
    for (Attribute refAttribute : refAttributes) {
      if (refAttribute.isVisible()
          && !isReferenceType(refAttribute)
        ...
        String value =
            refEntity.getString(refAttribute.getName()) == null
                ? ""
  ...
  private static void writeFormatString(BufferedWriter writer, Entity sample) throws IOException {
    ...
    if (formatColumn.length() > 0) {
      ...
      throw new MolgenisDataException(
          "Missing FORMAT information while trying to print first sample");
    }
  }
  ...
  private static boolean isOutputAttribute(
      Attribute attribute, List<Attribute> addedAttributes, List<String> attributesToInclude) {
    ...
    for (Attribute annotatorAttr : addedAttributes) {
      if (isReferenceType(annotatorAttr))
        expandedAddedAttributes.addAll(
            Lists.newArrayList(annotatorAttr.getRefEntity().getAtomicAttributes()));
      else expandedAddedAttributes.add(annotatorAttr);
    }

    List<String> annotatorAttributeNames =
        expandedAddedAttributes.stream().map(Attribute::getName).collect(Collectors.toList());
    ...
    return (!annotatorAttributeNames.contains(attribute.getName())
            || attributesToInclude.contains(attribute.getName())
            || attributesToInclude.isEmpty())
        && attribute.isVisible()
        && !attribute.getName().equals(VcfAttributes.SAMPLES);
  }
}
