...
public class ImportWizardController extends AbstractWizardController {
  ...
  @ResponseBody
  public Permissions getGroupEntityClassPermissions(
      @PathVariable String groupId, WebRequest webRequest) {
            ...
            entityClassId -> {
              ...
              if (value != null
                  && (SecurityUtils.currentUserHasRole(
                          SecurityUtils.AUTHORITY_ENTITY_WRITEMETA_PREFIX + entityClassId)
                      || userAccountService.getCurrentUser().isSuperuser())) {
                if (value.equalsIgnoreCase(READ.toString())
                    || value.equalsIgnoreCase(COUNT.toString())
                  ...
                  authority.setRole(
                      SecurityUtils.AUTHORITY_ENTITY_PREFIX
                          + value.toUpperCase()
                          + "_"
                          + entityClassId);
  ...
  private Permissions createPermissions(
      List<? extends Authority> entityAuthorities, List<EntityType> entityTypes) {
  ...
  private GroupAuthority getGroupAuthority(String groupId, String entityClassId) {
    ...
    for (Iterator<GroupAuthority> it = stream.iterator(); it.hasNext(); ) {
      ...
      if (groupAuthority.getRole().startsWith(SecurityUtils.AUTHORITY_ENTITY_COUNT_PREFIX)
        ...
        entity =
            groupAuthority.getRole().substring(SecurityUtils.AUTHORITY_ENTITY_READ_PREFIX.length());
      } else if (groupAuthority
          .getRole()
          .startsWith(SecurityUtils.AUTHORITY_ENTITY_WRITEMETA_PREFIX)) {
        entity =
            groupAuthority
                .getRole()
                .substring(SecurityUtils.AUTHORITY_ENTITY_WRITEMETA_PREFIX.length());
      }
  ...
  private ImportRun importFile(
      HttpServletRequest request, File file, String action, Boolean notify, String packageId) {
