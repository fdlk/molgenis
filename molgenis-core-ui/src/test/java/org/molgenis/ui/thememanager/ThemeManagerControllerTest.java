...
public class ThemeManagerControllerTest {
  ...
  public void before() {
    ...
    bs3File =
        new MockMultipartFile("bootstrap3-style", bootstrap3FileName, "some-content-type", bs3Data);
    bs4File =
        new MockMultipartFile("bootstrap4-style", bootstrap4FileName, "some-content-type", bs4Data);
  }
  ...
  public void addBootstrap3ThemeOnly() throws Exception {
    ...
    mockMvc
        .perform(fileUpload(ThemeManagerController.URI + "/add-bootstrap-theme").file(bs3File))
        .andExpect(status().isOk());

    verify(styleService)
        .addStyle(eq(bootstrap3FileName), eq(bootstrap3FileName), any(), any(), any());
  }
  ...
  public void addBootstrap3and4Theme() throws Exception {
    ...
    mockMvc
        .perform(
            fileUpload(ThemeManagerController.URI + "/add-bootstrap-theme")
                .file(bs3File)
                .file(bs4File)
