...
public class CaptchaControllerTest extends AbstractTestNGSpringContextTests {
  @Autowired private CaptchaController captchaController;
  ...
  public void getCaptcha() throws Exception {
    this.mockMvc
        .perform(get("/captcha").accept(MediaType.IMAGE_JPEG))
  ...
  public void validateCaptcha_valid() throws Exception {
    this.mockMvc
        .perform(
            post("/captcha")
                .content("{\"captcha\":\"captcha_answer\"}")
  ...
  public void validateCaptcha_invalid() throws Exception {
    this.mockMvc
        .perform(
            post("/captcha")
                .content("{\"captcha\":\"invalid_answer\"}")
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
