...
public class SemanticSearchServiceImplTest extends AbstractMolgenisSpringTest {
  ...
  @Mock
  private java.util.Set<org.molgenis.data.semanticsearch.explain.bean.ExplainedQueryString>
  ...
  public void testDistanceFrom() {
    ...
    Assert.assertEquals(
        semanticSearchService.distanceFrom(
            "Hypertension", ImmutableSet.of("history", "hypertens"), stemmer),
        .6923,
        0.0001,
        "String distance should be equal");
    Assert.assertEquals(
        semanticSearchService.distanceFrom(
            "Maternal Hypertension", ImmutableSet.of("history", "hypertens"), stemmer),
        .5454,
        0.0001,
        "String distance should be equal");
  }
  ...
  public void testFindAttributes() {
    ...
    Map<Attribute, ExplainedAttribute> termsActual1 =
        semanticSearchService.findAttributes(
            sourceEntityType, newHashSet("targetAttribute"), emptyList());

    Map<Attribute, ExplainedAttribute> termsExpected1 =
        ImmutableMap.of(attributeHeight, ExplainedAttribute.create(attributeHeight));
    ...
    Map<Attribute, ExplainedAttribute> termsActual2 =
        semanticSearchService.findAttributes(
            sourceEntityType, newHashSet("targetAttribute"), emptyList());
