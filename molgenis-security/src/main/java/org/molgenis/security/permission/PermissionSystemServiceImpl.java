...
public class PermissionSystemServiceImpl implements PermissionSystemService {
  ...
  private void giveUserEntityPermissionsAsSystem(
      SecurityContext securityContext, Collection<EntityType> entityTypes) {
  ...
  private Collection<GrantedAuthority> getGrantedAuthorities(
      Collection<EntityType> entityTypeStream) {
  ...
  private void updateUserAuthorities(
      SecurityContext context, Collection<GrantedAuthority> grantedAuthorities) {
    User user = userService.getUser(SecurityUtils.getUsername(context.getAuthentication()));
    Stream<UserAuthority> userAuthorityStream =
        grantedAuthorities
            .stream()
            .map(
                grantedAuthority -> {
  ...
  private void updateSecurityContext(
      SecurityContext context, Collection<? extends GrantedAuthority> authorities) {
    Collection<? extends GrantedAuthority> reachableAuthorities =
        roleHierarchy.getReachableGrantedAuthorities(authorities);

    List<GrantedAuthority> newGrantedAuthorities =
        Lists.newArrayList(context.getAuthentication().getAuthorities());
