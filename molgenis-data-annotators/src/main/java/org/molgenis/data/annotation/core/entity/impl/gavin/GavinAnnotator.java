...
public class GavinAnnotator implements AnnotatorConfig {
  ...
  @Autowired private Entity gavinAnnotatorSettings;
  ...
  @Autowired private EffectsMetaData effectsMetaData;
      ...
      public RepositoryFactory getRepositoryFactory() {
        return new InMemoryRepositoryFactory(
            ...
            new EmxMetaDataParser(
                packageFactory, attributeFactory, entityTypeFactory, entityTypeDependencyResolver),
            entityTypeFactory,
            attributeFactory);
      }
    };
    ...
    EntityAnnotator entityAnnotator =
        new QueryAnnotatorImpl(
            ...
            (annotationSourceFileName) ->
                gavinAnnotatorSettings.set(VARIANT_FILE_LOCATION, annotationSourceFileName)) {
          @Override
          ...
          public List<Attribute> getRequiredAttributes() {
            ...
            List<Attribute> refAttributesList =
                Arrays.asList(
                    createCaddScaledAttr(attributeFactory),
            ...
            requiredAttributes.addAll(
                Arrays.asList(
                    effectsMetaData.getGeneNameAttr(),
                    effectsMetaData.getPutativeImpactAttr(),
                    refAttr,
          ...
          protected void processQueryResults(
              Entity entity, Iterable<Entity> annotationSourceEntities, boolean updateMode) {
            if (updateMode) {
              throw new MolgenisDataException(
                  "This annotator/filter does not support updating of values");
            }
            ...
            Map<String, Double> caddMap =
                AnnotatorUtils.toAlleleMap(
                    variantEntity.getString(ALT), variantEntity.getString(CADD_SCALED));
            Map<String, Double> exacMap =
                AnnotatorUtils.toAlleleMap(
                    variantEntity.getString(ALT), variantEntity.getString(EXAC_AF));
            ...
            if (sourceEntitiesSize == 1) {
              ...
              Judgment judgment =
                  gavinAlgorithm.classifyVariant(
                      impact,
                      caddScaled,
                      exacMAF,
                      gene,
              ...
              Judgment judgment =
                  gavinAlgorithm.genomewideClassifyVariant(impact, caddScaled, exacMAF, gene);
              entity.set(CLASSIFICATION, judgment.getClassification().toString());
              entity.set(CONFIDENCE, judgment.getConfidence().toString());
              entity.set(REASON, judgment.getReason());
            } else {
