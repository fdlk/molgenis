...
public class SearchServiceIT extends AbstractTestNGSpringContextTests {
  ...
  public void testFuzzyMatch() {
    ...
    Query<Entity> query =
        new QueryImpl<>(new QueryRule(ATTR_XREF, FUZZY_MATCH, "\"0[0].1[1]\""))
            .and()
  ...
  public void testSemanticSearch() {
    ...
    List<String> queryTerms =
        asList(
            "hypertension",
            "disorder vascular hypertensive",
            "increased pressure blood",
            "high pressure blood",
            "ocular^0.5 hypertension^0.5",
    ...
    for (String term :
        asList(
            "hypertens",
            "disord vascular hypertens",
            "increased pressur blood",
            "high pressur blood",
            "ocular hypertens",
            "hypertens idiopathic pulmonary")) {
    ...
    List<Set<ExplainedQueryString>> expectedExplanationStrings =
        asList(
            // High chance of pulmonary disease
            singleton(
                ExplainedQueryString.create(
                    "high", "high pressur blood", "hypertension", 41.66666666666667)),
            ...
            singleton(
                ExplainedQueryString.create(
                    "high pressur blood", "high pressur blood", "hypertension", 100.0)),
            // Do you suffer from Ocular hypertension?
            singleton(
                ExplainedQueryString.create(
                    "ocular hypertens", "ocular hypertens", "hypertension", 100.0)),
  ...
  private static Object[][] findQueryOperatorLike() {
    return new Object[][] {
      {"ring", asList("0", "1")}, {"Ring", asList("0", "1")}, {"nomatch", emptyList()}
    };
  }
  ...
  public void testFindOneQuery() {
    ...
    Object entityId =
        searchService.searchOne(
            entityTypeDynamic, new QueryImpl<>().eq(ATTR_ID, entity.getIdValue()));
