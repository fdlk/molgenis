...
public class GoNLAnnotator implements AnnotatorConfig {
  ...
  @Autowired private Entity goNLAnnotatorSettings;
  ...
  public void init() {
    ...
    AnnotatorInfo thousandGenomeInfo =
    ...
    EntityAnnotator entityAnnotator =
        new QueryAnnotatorImpl(
            ...
            (annotationSourceFileName) -> {
              ...
              goNLAnnotatorSettings.set(
                  OVERRIDE_CHROMOSOME_FILES, "X:gonl.chrX.release4.gtc.vcf.gz");
              goNLAnnotatorSettings.set(
          ...
          protected void processQueryResults(
            ...
            for (Entity resourceEntity : annotationSourceEntities) {
              if (resourceEntity
                ...
                String newAltString =
                    Arrays.stream(
                            resourceEntity
                                .getString(vcfAttributes.getAltAttribute().getName())
                                .split(","))
                ...
                int postFixInputLength =
                    resourceEntity
                        .getString(vcfAttributes.getRefAttribute().getName())
                        .substring(
                            entity.getString(vcfAttributes.getRefAttribute().getName()).length())
                        .length();
                ...
                String newAltString =
                    Arrays.stream(
                        ...
                        .map(
                            alt ->
                                alt.length() > postFixInputLength
                                    ? alt.substring(0, (alt.length() - postFixInputLength))
                                    : "n/a")
                        .collect(Collectors.joining(","));
                resourceEntity.set(vcfAttributes.getAltAttribute().getName(), newAltString);
                refMatches.add(resourceEntity);
            ...
            if (entity.getString(vcfAttributes.getAltAttribute().getName()) != null) {
              ...
              for (String alt :
                  entity.getString(vcfAttributes.getAltAttribute().getName()).split(",")) {
                alleleMatches.add(
                    refMatches
                        .stream()
                        .filter(
              ...
              if (!alleleMatches.stream().allMatch(Predicates.isNull()::apply)) {
                        ...
                        .map(
                            gonl ->
                                gonl == null
                                    ? "."
                                    : Double.toString(
                                        Double.valueOf(gonl.getString(INFO_AC))
                                            / gonl.getInt(INFO_AN)))
                        .collect(Collectors.joining(","));
                ...
                gtcs =
                    alleleMatches
                        .stream()
                        .map(
                            gonl -> gonl == null ? "." : gonl.getString(INFO_GTC).replace(",", "|"))
                        .collect(Collectors.joining(","));
              }
            }
  ...
  Resource gonlresources() {
    MultiResourceConfig goNLConfig =
        new MultiResourceConfigImpl(
            CHROMOSOMES,
            FILEPATTERN,
            ROOT_DIRECTORY,
            OVERRIDE_CHROMOSOME_FILES,
    ...
    return new MultiFileResource(GONL_MULTI_FILE_RESOURCE, goNLConfig) {
      ...
      public RepositoryFactory getRepositoryFactory() {
        return new TabixVcfRepositoryFactory(
            GONL_MULTI_FILE_RESOURCE, vcfAttributes, entityTypeFactory, attributeFactory);
      }
    };
  }
}
