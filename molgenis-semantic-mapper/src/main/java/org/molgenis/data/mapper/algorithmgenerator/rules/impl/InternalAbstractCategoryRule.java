...
public abstract class InternalAbstractCategoryRule implements CategoryRule {
  ...
  @Override
  public CategoryMatchQuality<Double> createCategoryMatchQuality(
      Category targetCategory, Category sourceCategory) {
    String matchedTermForTargetLabel =
        getMatchedTermFromTheRulelabelContainsWords(targetCategory.getLabel());
    String matchedTermForSourceLabel =
        getMatchedTermFromTheRulelabelContainsWords(sourceCategory.getLabel());

    boolean ruleApplied =
        StringUtils.isNotBlank(matchedTermForTargetLabel)
            && StringUtils.isNotBlank(matchedTermForSourceLabel);

    Quality<Double> quality =
        NumericQuality.create(
  ...
  private double createNumericQualityIndicator(
      String matchedTermForTargetLabel, String matchedTermForSourceLabel) {
