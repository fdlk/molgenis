...
public class OmimRepository extends AbstractRepository {
  ...
  public OmimRepository(
      File file, EntityTypeFactory entityTypeFactory, AttributeFactory attributeFactory) {
    this.file = file;
  ...
  private List<Entity> getEntities() {
    ...
    getEntitiesByGeneSymbol()
        .forEach((geneSymbol, geneSymbolEntities) -> entities.addAll(geneSymbolEntities));
    return entities;
  }
  ...
  private Map<String, List<Entity>> getEntitiesByGeneSymbol() {
    if (entitiesByGeneSymbol == null) {
      ...
      try (CSVReader csvReader =
          new CSVReader(
              new InputStreamReader(new FileInputStream(file), forName("UTF-8")),
              SEPARATOR,
  ...
  private void addEntityToGeneEntityList(
    ...
    List<Entity> entities =
        entitiesByGeneSymbol.computeIfAbsent(geneSymbol, k -> new ArrayList<>());
    entities.add(entity);
  }
  ...
  private void addLineToMap(
      Map<String, List<List<String>>> omimEntriesByGeneSymbol, String[] values) {
    ...
    if (entry.matches("[0-9]+")) {
      ...
      for (String geneSymbol : genes) {
        if (omimEntriesByGeneSymbol.containsKey(geneSymbol)) {
          ...
          omimEntriesByGeneSymbol
              .get(geneSymbol)
              .get(1)
              .add(causalIdentifier); // second is mim number
          omimEntriesByGeneSymbol
              .get(geneSymbol)
              .get(2)
              .add(cytogenicLocation); // third is cyto location
          omimEntriesByGeneSymbol.get(geneSymbol).get(3).add(type); // fourth is type of syndrome
          omimEntriesByGeneSymbol
              .get(geneSymbol)
              .get(4)
              .add(omimEntry); // fifth is omim entry location
        } else {
