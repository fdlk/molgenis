...
/**
 * This class is used to convert from (MOLGENIS)SnpEff annotated VCF files to the entity structure
 * expected by Effects annotators (for example GAVIN), and the other way around
 *
 * <p>An effect is an entity containing information about the combination of a variant (CHROM POS
 * REF ALT(single allele)) in a specific GENE
 */
@Component
public class EffectStructureConverter {
  ...
  @Autowired
  public EffectStructureConverter(
  ...
  public Iterator<Entity> createVcfEntityStructure(Iterator<Entity> annotatedRecords) {
    return new Iterator<Entity>() {
      ...
      private Entity createVcfEntityStructureForSingleEntity(
          Entity variant, List<Entity> effectsForVariant) {
  ...
  public Stream<Entity> createVariantEffectStructure(
      String effectAttributeName,
    ...
    EntityType variantEntityType =
        removeAttributeAndCreateEntityTypeCopy(
    ...
    ArrayList<Attribute> effectFieldAttributeList =
        parseEffectAttributeDescription(attributesString, annotatorAttributes);
    EntityType effectsEntityType =
        createEffectsEntityType(effectFieldAttributeList, effectEntityName, annotatorAttributes);

    return StreamSupport.stream(vcfRepository.spliterator(), false)
        .flatMap(
            entity ->
                createVariantEffectStructureForSingleEntity(
                    effectsAttribute,
                    effectFieldAttributeList,
                    effectsEntityType,
                    entity,
                    variantEntityType));
  }

  private EntityType removeAttributeAndCreateEntityTypeCopy(
      Attribute attributeToParse, EntityType inputEntityType) {
    EntityType newMeta = EntityType.newInstance(inputEntityType);
    newMeta.removeAttribute(attributeToParse);
    return newMeta;
  }

  private Stream<Entity> createVariantEffectStructureForSingleEntity(
      Attribute attributeToParse,
      ArrayList<Attribute> effectFieldAttributeList,
      EntityType effectsEntityType,
      Entity vcfInputEntity,
      EntityType variantEntityType) {
    ...
    List<Entity> result =
        createEffectsEntitiesForSingleVariant(
                effectsEntityType,
                effectFieldAttributeList,
                vcfInputEntity.getString(attributeToParse.getName()),
                variantEntity)
  ...
  public String getEffectDescription(Attribute effectAttributeToParse) {
    ...
    if (description.indexOf(':') == -1) {
      throw new RuntimeException(
          "Unable to create entitystructure, missing semicolon in description of ["
  ...
  private EntityType createEffectsEntityType(
      ArrayList<Attribute> effectFieldAttributeList,
      String effectEntityName,
      List<Attribute> annotatorAttributes) {
    EntityType effectsEntityType = entityTypeFactory.create(effectEntityName);
    effectsEntityType.addAttribute(
        attributeFactory.create().setName("identifier").setAuto(true).setVisible(false),
  ...
  private void addAnnotatorAttributes(
      List<Attribute> annotatorAttributes, EntityType effectsEntityType) {
  ...
  private ArrayList<Attribute> parseEffectAttributeDescription(
      String attributesString, List<Attribute> annotatorAttributes) {
    String[] attributeStrings = attributesString.replaceAll("^\\s'|'$", "").split("\\|");
    ArrayList<Attribute> attributeList = new ArrayList<>();
    Map<String, Attribute> annotatorAttributeMap =
        VcfUtils.getAttributesMapFromList(annotatorAttributes);
    for (String attribute : attributeStrings) {
      AttributeType type =
          annotatorAttributeMap.containsKey(attribute)
  ...
  private static Stream<Entity> createEffectsEntitiesForSingleVariant(
      EntityType effectsEntityType,
      List<Attribute> effectFieldAttributeList,
      String descriptionFieldsString,
