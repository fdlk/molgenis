...
public abstract class OntologyTerm {
  ...
  public static OntologyTerm create(String iri, String label, String description) {
    return new AutoValue_OntologyTerm(
        iri, label, description, Collections.singletonList((description)));
  }

  public static OntologyTerm create(
      String iri, String label, String description, List<String> synonyms) {
    return new AutoValue_OntologyTerm(iri, label, description, copyOf(synonyms));
  }

  /**
   * Creates a new {@link OntologyTerm} that is the combined ontology term of a couple ontology
   * terms. Its IRI will be a comma separated list of all original ontology term IRIs. Its label
   * will be for instance <code>(term1 and term2 and term3)</code>. It won't have any synonyms.
   *
   * <p>This can be used to tag an attribute with (hypertension and medication).
   *
