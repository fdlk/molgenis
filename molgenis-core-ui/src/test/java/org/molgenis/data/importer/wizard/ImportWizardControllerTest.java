...
import org.molgenis.data.importer.ImportStatus;
...
public class ImportWizardControllerTest extends AbstractMolgenisSpringTest {
  ...
  @Autowired private GrantedAuthoritiesMapper grantedAuthoritiesMapper;

  @Autowired private UserAccountService userAccountService;

  @Autowired private ImportRunFactory importRunFactory;

  @Autowired private GroupFactory groupFactory;

  @Autowired private GroupAuthorityFactory groupAuthorityFactory;

  @Autowired private MetaDataService metaDataService;

  @Captor private ArgumentCaptor<GroupAuthority> groupAuthorityArgumentCaptor;
  ...
  public void setUp() throws ParseException {
    ...
    entity1.set(
        AuthorityMetaData.ROLE, SecurityUtils.AUTHORITY_ENTITY_WRITEMETA_PREFIX + "entity1");
    ...
    entity2.set(
        AuthorityMetaData.ROLE, SecurityUtils.AUTHORITY_ENTITY_WRITEMETA_PREFIX + "entity2");
    ...
    entity3.set(
        AuthorityMetaData.ROLE, SecurityUtils.AUTHORITY_ENTITY_WRITEMETA_PREFIX + "entity3");
    ...
    entity4.set(
        AuthorityMetaData.ROLE, SecurityUtils.AUTHORITY_ENTITY_WRITEMETA_PREFIX + "entity4");
  ...
  public void addGroupEntityClassPermissionsTestNoPermissionSU() {
    ...
    assertEquals(
        groupAuthorityArgumentCaptor.getValue().getGroup(),
  ...
  public void testImportFile() throws IOException, URISyntaxException {
    ...
    when(fileRepositoryCollectionFactory.createFileRepositoryCollection(file))
        .thenReturn(repositoryCollection);
    ...
    when(importRunService.addImportRun(SecurityUtils.getCurrentUsername(), false))
        .thenReturn(importRun);

    // the actual test
    ResponseEntity<String> response =
        controller.importFile(request, multipartFile, null, null, "add", null);
  ...
  public void testImportUpdateFile() throws IOException, URISyntaxException {
    ...
    // the actual test
    ResponseEntity<String> response =
        controller.importFile(request, multipartFile, null, null, "update", null);
    assertEquals(response.getStatusCode(), HttpStatus.CREATED);
  ...
  public void testImportIllegalUpdateModeFile() throws IOException, URISyntaxException {
    ...
    // the actual test
    ResponseEntity<String> response =
        controller.importFile(request, multipartFile, null, null, "addsss", null);
