...
public class UserManagerServiceImpl implements UserManagerService {
  ...
  public List<Group> getGroupsWhereUserIsNotMember(final String userId) {
    ...
    final List<Group> groupsWhereUserIsMember =
        this.getAllMolgenisGroupsFromGroupMembers(groupMembers);
  ...
  public void removeUserFromGroup(String molgenisGroupId, String molgenisUserId) {
    ...
    final List<GroupMember> groupMembers =
        dataService.findAll(GROUP_MEMBER, q, GroupMember.class).collect(toList());
  ...
  private List<UserViewData> parseToMolgenisUserViewData(Stream<User> users) {
    return users
        .map(user -> new UserViewData(user, getMolgenisGroups(user.getId())))
        .collect(toList());
  }
}
