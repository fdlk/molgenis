...
public class AppsControllerTest extends AbstractMockitoTestNGSpringContextTests {
  ...
  public void setUpBeforeMethod() {
    ...
    mockMvc =
        MockMvcBuilders.standaloneSetup(appsController)
  ...
  public void testInit(boolean hasWriteAppPermission) throws Exception {
    ...
    when(permissionService.hasPermissionOnEntityType(APP, Permission.WRITE))
        .thenReturn(hasWriteAppPermission);

    AppInfoDto appInfoDto0 =
        AppInfoDto.builder()
            .setId("id0")
            .setName("name0")
            ...
            .build();
    AppInfoDto appInfoDto1 =
        AppInfoDto.builder().setId("id1").setName("name1").setActive(false).build();

    ResultActions resultActions =
        mockMvc
            .perform(get(AppsController.URI))
  ...
  public void testViewAppWithFreeMarkerTemplate() throws Exception {
    ...
    AppInfoDto expectedAppInfo =
        AppInfoDto.builder().setId("id").setName("name").setActive(true).build();
    mockMvc
        .perform(get(AppsController.URI + "/id"))
        .andExpect(status().isOk())
  ...
  public void testActivateAppUnknownApp() throws Exception {
    App app = when(mock(App.class).isActive()).thenReturn(false).getMock();
    mockMvc
        .perform(post(AppsController.URI + "/id/activate"))
        .andExpect(status().isBadRequest())
  ...
  public void testActivateAppAlreadyActivated() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(status().isBadRequest())
        .andExpect(
            content().string("{\"errors\":[{\"message\":\"App 'name' already activated\"}]}"));
  }
  ...
  public void testDeactivateAppUnknownApp() throws Exception {
    App app = when(mock(App.class).isActive()).thenReturn(true).getMock();
    mockMvc
        .perform(post(AppsController.URI + "/id/deactivate"))
        .andExpect(status().isBadRequest())
        .andExpect(content().string("{\"errors\":[{\"message\":\"Unknown app 'id'\"}]}"));
  }
  ...
  public void testDeActivateAppAlreadyDeactivated() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(status().isBadRequest())
        .andExpect(
            content().string("{\"errors\":[{\"message\":\"App 'name' already deactivated\"}]}"));
  }
}
