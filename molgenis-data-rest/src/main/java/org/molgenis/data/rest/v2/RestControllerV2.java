...
public class RestControllerV2 {
  ...
  static UnknownAttributeException createUnknownAttributeException(
      String entityTypeId, String attributeName) {
    return new UnknownAttributeException(
        "Operation failed. Unknown attribute: '"
            + attributeName
            + "', of entity: '"
            + entityTypeId
            + "'");
  ...
  private Map<String, Object> getEntityResponse(
      String entityTypeId, String untypedId, AttributeFilter attributeFilter) {
    ...
    try {
      ...
      responseBody.setLocation(
          Href.concatEntityCollectionHref(
              RestControllerV2.BASE_URI, entityTypeId, meta.getIdAttribute().getName(), ids));
  ...
  public String copyEntity(
    ...
    boolean readPermission =
        permissionService.hasPermissionOnEntityType(
            repositoryToCopyFrom.getName(), Permission.READ);
    ...
    Repository<Entity> repository =
        this.copyRepositoryRunAsSystem(
  ...
  private Repository<Entity> copyRepositoryRunAsSystem(
      Repository<Entity> repositoryToCopyFrom, String entityTypeId, Package pack, String label) {
    return runAsSystem(
        () -> repoCopier.copyRepository(repositoryToCopyFrom, entityTypeId, pack, label));
  }
  ...
  private AttributeResponseV2 createAttributeResponse(String entityTypeId, String attributeName) {
    ...
    return new AttributeResponseV2(
        entityTypeId, entity, attribute, null, permissionService, dataService, languageService);
  }

  private EntityCollectionResponseV2 createEntityCollectionResponse(
      String entityTypeId, EntityCollectionRequestV2 request, HttpServletRequest httpRequest) {
      ...
      AttributeResponseV2 xAttrResponse =
          xAttr != null
              ? new AttributeResponseV2(
                  entityTypeId, meta, xAttr, fetch, permissionService, dataService, languageService)
              : null;
      AttributeResponseV2 yAttrResponse =
          yAttr != null
              ? new AttributeResponseV2(
                  entityTypeId, meta, yAttr, fetch, permissionService, dataService, languageService)
              : null;
      return new EntityAggregatesResponse(
          aggs, xAttrResponse, yAttrResponse, BASE_URI + '/' + entityTypeId);
    } else {
  ...
  private void createEntityTypeResponse(
      EntityType entityType, Fetch fetch, Map<String, Object> responseData) {
    responseData.put(
        "_meta",
        new EntityTypeResponseV2(
            entityType, fetch, permissionService, dataService, languageService));
  }
