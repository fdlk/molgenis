...
public class QuestionnairePluginController extends PluginController {
  ...
  @Autowired
  public QuestionnairePluginController(
  ...
  private List<Questionnaire> getQuestionnaires() {
    ...
    List<EntityType> questionnaireMeta =
        runAsSystem(() -> findQuestionnairesMetaData(dataService).collect(toList()));
    ...
    questionnaires =
        questionnaireMeta
            ...
            .filter(
                name ->
                    currentUserIsSu() || currentUserHasRole(AUTHORITY_ENTITY_WRITE_PREFIX + name))
            .map(
                name -> {
  ...
  @RequestMapping("/{name}")
  public String showQuestionnairForm(
      @PathVariable("name") String name, Model model, HttpServletResponse response)
      throws IOException {
    EntityType entityType = dataService.getMeta().getEntityType(name);
    if (entityType == null) {
  ...
  private synchronized Entity createQuestionnaireEntity(
      EntityType entityType, QuestionnaireStatus status, String name) {
    Entity entity = findQuestionnaireEntity(name);
    if (entity == null) {
  ...
  private Questionnaire toQuestionnaireModel(Entity entity, EntityType entityType) {
    ...
    return new Questionnaire(
        entityType.getId(),
        entityType.getLabel(languageService.getCurrentUserLanguageCode()),
        status,
        entityType.getDescription(languageService.getCurrentUserLanguageCode()),
        entity.getIdValue());
  }
  ...
  public static class Questionnaire {
    ...
    Questionnaire(
        String name, String label, QuestionnaireStatus status, String description, Object id) {
      this.name = name;
      this.label = label;
      this.status = status;
