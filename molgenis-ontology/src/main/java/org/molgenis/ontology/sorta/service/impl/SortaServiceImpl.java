...
public class SortaServiceImpl implements SortaService {
  private static final Set<String> ELASTICSEARCH_RESERVED_WORDS =
      Sets.newHashSet("or", "and", "if");
  ...
  public Iterable<Entity> findOntologyTermEntities(String ontologyIri, Entity inputEntity) {
    ...
    if (rulesForOtherFields.size() > 0) {
      annotationMatchOntologyTerms(
          inputEntity, ontologyEntity, relevantEntities, rulesForOtherFields);
    }
    ...
    relevantEntities.sort(
        (entity_1, entity_2) ->
            entity_2.getDouble(COMBINED_SCORE).compareTo(entity_1.getDouble(COMBINED_SCORE)));

    return relevantEntities;
  }

  private void annotationMatchOntologyTerms(
      Entity inputEntity,
      Entity ontologyEntity,
      List<Entity> relevantEntities,
      List<QueryRule> rulesForOtherFields) {
    List<Entity> ontologyTermAnnotationEntities =
        dataService
            .findAll(
    ...
    if (ontologyTermAnnotationEntities.size() > 0) {
      ...
      Stream<Entity> ontologyTermEntities =
          dataService.findAll(ONTOLOGY_TERM, new QueryImpl<>(rules).pageSize(Integer.MAX_VALUE));

      List<Entity> relevantOntologyTermEntities =
          ontologyTermEntities
              .map(
                  ontologyTermEntity ->
                      calculateNGromOTAnnotations(inputEntity, ontologyTermEntity))
              .collect(Collectors.toList());
  ...
  private void lexicalMatchOntologyTerms(
    ...
    Stream<Entity> lexicalMatchedOntologyTermEntities =
        dataService
            .findAll(ONTOLOGY_TERM, new QueryImpl<>(finalQueryRules).pageSize(pageSize))
            .map(
                ontologyTerm ->
                    addLexicalScoreToMatchedEntity(inputEntity, ontologyTerm, ontologyIri));

    lexicalMatchedOntologyTermEntities.forEach(
        matchedEntity -> {
  ...
  Entity addLexicalScoreToMatchedEntity(
      Entity inputEntity, Entity ontologyTerm, String ontologyIri) {
  ...
  private Entity findSynonymWithHighestNgramScore(
    ...
    if (Iterables.size(entities) > 0) {
      ...
      List<Entity> synonymEntities =
          FluentIterable.from(entities)
              .transform(
                  ontologyTermSynonymEntity -> {
                    ...
                    mapEntity.set(
                        ...
                        NGramDistanceAlgorithm.stringMatching(
                            cleanedQueryString, ontologyTermSynonym));
                    return mapEntity;
                  })
              .toSortedList(
                  (entity_1, entity_2) ->
                      entity_2.getDouble(SCORE).compareTo(entity_1.getDouble(SCORE)));

      Entity firstMatchedSynonymEntity =
          Iterables.getFirst(synonymEntities, ontologyTermSynonymFactory.create());
      ...
      for (Entity nextMatchedSynonymEntity : Iterables.skip(synonymEntities, 1)) {
        ...
        tempCombinedSynonym
            .append(topMatchedSynonym)
            .append(SINGLE_WHITESPACE)
            .append(nextMatchedSynonym);
      ...
      // // weight.
      Map<String, Double> weightedWordSimilarity =
          informationContentService.redistributedNGramScore(cleanedQueryString, ontologyIri);

      Set<String> synonymStemmedWords =
          informationContentService.createStemmedWordSet(topMatchedSynonym);
  ...
  /**
   * A helper function to produce fuzzy match query with 80% similarity in elasticsearch because
