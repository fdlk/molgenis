...
public class AnnotationJob extends TransactionalJob<Void> {
  ...
  public Void call(Progress progress) throws Exception {
    ...
    if (firstException != null) {
      progress.status(
          "Failed annotators: "
              + StringUtils.join(failedAnnotators, ",")
              + ". Successful annotators: "
  ...
  private String getMessage() {
    return String.format(
        "Annotated \"%s\" with %s (started by \"%s\")",
        repository.getEntityType().getLabel(),
        StringUtils.join(successfulAnnotators, ","),
        username);
  }
  ...
  private String getMessage(int i, RepositoryAnnotator annotator) {
    return String.format(
        "Annotating \"%s\" with %s (annotator %d of %d, started by \"%s\")",
        repository.getEntityType().getLabel(),
