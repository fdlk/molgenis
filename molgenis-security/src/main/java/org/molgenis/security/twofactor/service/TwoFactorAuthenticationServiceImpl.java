...
public class TwoFactorAuthenticationServiceImpl implements TwoFactorAuthenticationService {
  ...
  public TwoFactorAuthenticationServiceImpl(
    ...
    if (!userIsBlocked()) {
      try {
        ...
        updateFailedLoginAttempts(
            userSecret.getFailedLoginAttempts() + FAILED_LOGIN_ATTEMPT_ITERATION);
        if (!userIsBlocked()) {
          throw err;
        }
      }
    ...
    if (userSecret.getFailedLoginAttempts() >= MAX_FAILED_LOGIN_ATTEMPTS) {
      if (userSecret.getLastFailedAuthentication() != null
        ...
        throw new TooManyLoginAttemptsException(
            format(
                "You entered the wrong verification code %d times, please wait for %d seconds before you try again",
  ...
  /**
   * Check if user has 3 or more failed login attempts -> then determine if the user is within the
   * 30 seconds of the last failed login attempt -> if the user is not outside the timeframe than
   * the failed login attempts are set to 1 because it is a failed login attempt When the user has
   * less than 3 failed login attempts -> the last failed login attempt is logged
   *
  ...
  private UserSecret getSecret() throws InternalAuthenticationServiceException {
    ...
    if (secret != null) {
      ...
      throw new InternalAuthenticationServiceException(
          format(
              "Secret not found, user: [ %s ] is not configured for 2 factor authentication",
              user.getUsername()));
    }
  }
  ...
  private User getUser() {
    ...
    if (user != null) {
      ...
      throw new UsernameNotFoundException(
          format("Can't find user: [ %s ]", SecurityUtils.getCurrentUsername()));
    }
  }
}
