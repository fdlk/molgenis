...
public class MetaDataSearchServiceImpl implements MetaDataSearchService {
  ...
  public List<PackageSearchResultItem> findRootPackages(String searchTerm) {
    ...
    if (StringUtils.isBlank(searchTerm)) {
      ...
      dataService
          .findAll(PACKAGE, packageQuery, Package.class)
          .forEach(
              packageEntity -> {
      ...
      dataService
          .findAll(ENTITY_TYPE_META_DATA, entityTypeQuery, EntityType.class)
          .forEach(
              entityType -> {
                Package p = getRootPackage(entityType);
                if (p != null) {
                  String matchDesc =
                      "Matched: entity '" + entityType.getString(EntityTypeMetadata.ID) + "'";
                  PackageSearchResultItem item =
                      new PackageSearchResultItem(p.getRootPackage(), matchDesc);
