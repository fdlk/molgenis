...
public class OmimRepositoryTest extends AbstractMolgenisSpringTest {
  ...
  public void findAllWithQuery() throws IOException {
    Iterator<Entity> it =
        repo.findAll(new QueryImpl<>().eq(OMIM_GENE_SYMBOLS_COL_NAME, "CYP17A1")).iterator();
    assertTrue(EntityUtils.equals(it.next(), entity2));
  }

  @Test(
    expectedExceptions = MolgenisDataException.class,
    expectedExceptionsMessageRegExp = "The only query allowed on this Repository is gene EQUALS"
  )
