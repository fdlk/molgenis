 ...
 *
 * <p>Reads the owl file's contents using an {@link OntologyLoader}. Fills {@link
 * InMemoryRepository}s with their contents.
 */
public class OntologyRepositoryCollection extends FileRepositoryCollection {
  ...
  @Autowired private OntologyTermNodePathFactory ontologyTermNodePathFactory;
  ...
  public OntologyRepositoryCollection(File file) {
    ...
    if (name.endsWith(OntologyFileExtensions.OBO_ZIP.toString())) {
      name =
          name.substring(0, name.lastIndexOf('.' + OntologyFileExtensions.OBO_ZIP.toString()))
              .replace('.', '_');
    } else if (name.endsWith(OntologyFileExtensions.OWL_ZIP.toString())) {
      name =
          name.substring(0, name.lastIndexOf('.' + OntologyFileExtensions.OWL_ZIP.toString()))
              .replace('.', '_');
    } else {
      throw new IllegalArgumentException(
          format("Not a obo.zip or owl.zip file [%s]", file.getName()));
    }
    this.fileName = name;
  }
  ...
  public void init() throws IOException {
    ...
    annotationRepository =
        new InMemoryRepository(ontologyTermDynamicAnnotationFactory.getEntityType());
   ...
   * @return the created {@link Entity}
   */
  private OntologyTermNodePath createNodePathEntity(
      OWLClassContainer container, String ontologyTermNodePathText) {
