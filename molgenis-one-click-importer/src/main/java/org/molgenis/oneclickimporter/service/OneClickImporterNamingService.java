...
/**
 * Service that offers methods to: - Generate valid ids by removing illegal characters - Generate
...
public interface OneClickImporterNamingService {
   ...
   *
   * <p>if the database contains an EntityType with label: 'label' then uploading a CSV file with
   * the name label.csv result in the newly created EntityType being labeled with label (1)
   *
   * @param label
  ...
  /**
   * Valid Identifiers are not allowed to contain illegal characters This method generates a valid
   * ID from a file name by replacing illegal characters with '_'
   *
   * <p>e.g. file-!!name.csv becomes file___name
   *
   * <p>Illegal character rules can be found in @{@link NameValidator#checkForIllegalCharacters}
   *
   * @param filename
