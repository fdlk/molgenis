...
public class AlgorithmServiceImplIT extends AbstractMolgenisSpringTest {
  ...
  public void testCreateAttributeMappingIfOnlyOneMatch() {
    ...
    Map<Attribute, ExplainedAttribute> matches =
        ImmutableMap.of(
            sourceAttribute,
            ExplainedAttribute.create(
                sourceAttribute,
                singletonList(ExplainedQueryString.create("height", "height", "height", 100)),
                true));

    LinkedHashMultimap<Relation, OntologyTerm> ontologyTermTags = LinkedHashMultimap.create();

    when(semanticSearchService.decisionTreeToFindRelevantAttributes(
            sourceEntityType, targetAttribute, ontologyTermTags.values(), null))
        .thenReturn(matches);
    ...
    algorithmService.autoGenerateAlgorithm(
        sourceEntityType, targetEntityType, mapping, targetAttribute);
  ...
  public void testWhenSourceDoesNotMatchThenNoMappingGetsCreated() {
    ...
    when(semanticSearchService.findAttributes(
            sourceEntityType, Sets.newHashSet("targetHeight", "height"), Collections.emptyList()))
        .thenReturn(emptyMap());
  ...
  public void testWhenSourceHasMultipleMatchesThenFirstMappingGetsCreated() {
    ...
    Map<Attribute, ExplainedAttribute> mappings =
        ImmutableMap.of(
            sourceAttribute1,
            ExplainedAttribute.create(sourceAttribute1),
            sourceAttribute2,
