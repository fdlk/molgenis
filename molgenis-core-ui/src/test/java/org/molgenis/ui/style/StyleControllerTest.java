...
public class StyleControllerTest {
  ...
  public void testGetTheme() throws Exception {
    ...
    when(styleService.getThemeData(
            "bootstrap-molgenis.min.css", BootstrapVersion.BOOTSTRAP_VERSION_3))
        .thenReturn(themeResource);

    mockMvc
        .perform(get("/css/bootstrap-3/bootstrap-molgenis.min.css").with(anonymous()))
  ...
  public void testGetBs4Theme() throws Exception {
    ...
    when(themeResource.getInputStream()).thenReturn(inputStream);
    when(styleService.getThemeData(
            "bootstrap-molgenis.min.css", BootstrapVersion.BOOTSTRAP_VERSION_4))
        .thenReturn(themeResource);
