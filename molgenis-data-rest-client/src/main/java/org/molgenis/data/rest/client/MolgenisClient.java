...
public class MolgenisClient {
  ...
  public LoginResponse login(String uid, String pwd) {
    ResponseEntity<LoginResponse> result =
        template.postForEntity(
            "{apiHref}/login", LoginRequest.create(uid, pwd), LoginResponse.class, apiHref);
  ...
  public ResponseEntity<MetaDataResponse> getMeta(String token, String entityTypeId) {
    return template.exchange(
        "{apiHref}/{entityTypeId}/meta?_method=GET",
        POST,
        createHttpEntity(token, MetaDataRequest.create()),
   ...
   * @param token molgenis session token
   * @return {@link ResponseEntity} with a {@link QueryResponse} body containing the entities which
   *     have attributeName set to value
   */
  public QueryResponse queryEquals(
      String token, String entityTypeId, String attributeName, Object value) {
    Map<String, Object> query =
        of("q", singletonList(of("field", attributeName, "operator", "EQUALS", "value", value)));
    ResponseEntity<QueryResponse> response =
        template.exchange(
            "{apiHref}/{entityTypeId}?_method=GET",
            POST,
            createHttpEntity(token, query),
            QueryResponse.class,
            apiHref,
            entityTypeId);
   ...
   */
  public void update(
      String token, String entityTypeId, String id, ImmutableMap<String, Object> newEntity) {
    template.exchange(
        "{apiHref}/{entityTypeId}/{id}",
        PUT,
        createHttpEntity(token, newEntity),
        Object.class,
        apiHref,
        entityTypeId,
        id);
   ...
   */
  public void update(
      String token, String entityTypeId, String id, String attributeName, Object value) {
    template.exchange(
        "{apiHref}/{entityTypeId}/{id}/{attributeName}",
        PUT,
        createHttpEntity(token, value),
        Object.class,
        apiHref,
        entityTypeId,
        id,
