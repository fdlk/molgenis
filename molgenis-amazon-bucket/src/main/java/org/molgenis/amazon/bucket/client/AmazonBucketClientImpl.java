...
public class AmazonBucketClientImpl implements AmazonBucketClient {
  ...
  private File storeFile(
    ...
    String filename;
    if (StringUtils.isNotEmpty(targetEntityName)
        && !ExcelFileExtensions.getExcel().contains(extension)) {
      filename =
          String.format("%s%s%s.%s", relativePath, File.separatorChar, targetEntityName, extension);
    } else {
