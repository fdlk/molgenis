...
public class GsonFactoryBean implements FactoryBean<Gson>, InitializingBean {
  ...
  /**
   * Indicates if the java 8 date time {@link com.fatboyindustrial.gsonjavatime.Converters} should
   * be registered with Gson.
   *
   * <p>This is equivalent to writing <code>
   * GsonBuilder builder = new GsonBuilder()
   * Converters.registerInstant(builder);
   * Converters.registerLocalDate(builder);
   * builder.create();
   * </code> The builders will be registered after the other type adapters and type adapter
   * factories you set.
   *
