...
public class MolgenisUiMenuPermissionDecorator implements UiMenu {
  ...
  public MolgenisUiMenuPermissionDecorator(
      UiMenu molgenisUiMenu, PermissionService permissionService) {
  ...
  public List<UiMenuItem> getItems() {
    return Lists.newArrayList(
        Iterables.filter(
            Iterables.transform(
                molgenisUiMenu.getItems(),
                molgenisUiMenuItem -> {
                  switch (molgenisUiMenuItem.getType()) {
                    case MENU:
                      return new MolgenisUiMenuPermissionDecorator(
                          (UiMenu) molgenisUiMenuItem, permissionService);
                    case PLUGIN:
  ...
  public boolean containsItem(final String itemId) {
    return Iterables.any(
        molgenisUiMenu.getItems(),
        molgenisUiMenuItem ->
            molgenisUiMenuItem.getId().equals(itemId) && hasPermission(molgenisUiMenuItem));
  }
  ...
  public UiMenuItem getActiveItem() {
    return Iterables.find(
        molgenisUiMenu.getItems(),
        molgenisUiMenuItem ->
            molgenisUiMenuItem.getType() != UiMenuItemType.MENU
                && hasPermission(molgenisUiMenuItem),
        null);
  }
