...
public class EntityTestHarness {
  ...
  private EntityType createDynamicTestEntityType(EntityType entityType, EntityType refEntityType) {
    entityType
        ...
        .addAttribute(
            createAttribute(ATTR_BOOL, BOOL)
        ...
        .addAttribute(
            createAttribute(ATTR_CATEGORICAL_MREF, CATEGORICAL_MREF).setRefEntity(refEntityType))
        .addAttribute(createAttribute(ATTR_DATE, DATE))
        .addAttribute(
            createAttribute(ATTR_ENUM, ENUM)
  ...
  public List<Entity> createTestRefEntities(EntityType refEntityType, int numberOfEntities) {
    return IntStream.range(0, numberOfEntities)
        .mapToObj(i -> createRefEntity(refEntityType, i))
        .collect(toList());
  }

  public Stream<Entity> createTestEntities(
  ...
  private Entity createEntity(EntityType entityType, int id, Entity refEntity) {
    ...
    entity.set(
        ATTR_HTML, id % 2 == 1 ? "<html>where is my head and where is my body</html>" : null);
