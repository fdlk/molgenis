...
public class StaticContentServiceImplTest extends AbstractTestNGSpringContextTests {
  ...
  @Test(
    ...
    expectedExceptionsMessageRegExp = "No write permissions on static content page"
  )
  ...
  private void setSecurityContextSuperUser() {
    ...
    UserDetails userDetails =
        when(mock(UserDetails.class).getUsername())
            .thenReturn(SecurityUtils.AUTHORITY_SU)
            .getMock();
  ...
  private void setSecurityContextNonSuperUser(Boolean hasPermissions) {
    ...
    when(authentication.isAuthenticated()).thenReturn(true);
    UserDetails userDetails =
        when(mock(UserDetails.class).getUsername()).thenReturn("user").getMock();
    when(authentication.getPrincipal()).thenReturn(userDetails);
  ...
  private void setSecurityContextAnonymousUsers() {
    ...
    UserDetails userDetails =
        when(mock(UserDetails.class).getUsername())
            .thenReturn(SecurityUtils.ANONYMOUS_USERNAME)
            .getMock();
    when(authentication.getPrincipal()).thenReturn(userDetails);
  ...
  public static class Config extends WebSecurityConfigurerAdapter {
    ...
    public StaticContentService staticContentService() {
      return new StaticContentServiceImpl(
          dataService(), staticContentFactory(), permissionService());
    }
    ...
    public DataService dataService() {
      ...
      StaticContent staticContent =
          when(mock(StaticContent.class).getContent())
