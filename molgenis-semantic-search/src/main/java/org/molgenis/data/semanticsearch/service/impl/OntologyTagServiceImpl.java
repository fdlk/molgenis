...
public class OntologyTagServiceImpl implements OntologyTagService {
  ...
  public void removeAttributeTag(
    ...
    Iterable<Entity> newTags =
        Iterables.filter(tags, e -> !isSameTag(relationIRI, ontologyTermIRI, e));
  ...
  @Override
  public void removeAttributeTag(
      EntityType entityType, SemanticTag<Attribute, OntologyTerm, Ontology> removeTag) {
  ...
  @Override
  @RunAsSystem
  public Multimap<Relation, OntologyTerm> getTagsForAttribute(
  ...
  @Override
  public OntologyTag addAttributeTag(
      String entity, String attribute, String relationIRI, List<String> ontologyTermIRIs) {
    boolean added = false;
    Entity attributeEntity = findAttributeEntity(entity, attribute);
  ...
  public void removeAllTagsFromEntity(String entityTypeId) {
    ...
    for (Attribute attribute : attributes) {
      ...
      dataService.update(ATTRIBUTE_META_DATA, attributeEntity);
      updateEntityTypeEntityWithNewAttributeEntity(
          entityTypeId, attribute.getName(), attributeEntity);
    }
  }

  @Override
  @Transactional
  public Map<String, OntologyTag> tagAttributesInEntity(
      String entity, Map<Attribute, OntologyTerm> tags) {
  ...
  /**
   * The attribute just got updated, but the entity does not know this yet. To reindex this document
  ...
  private void updateEntityTypeEntityWithNewAttributeEntity(
    ...
    EntityType entityEntity =
        dataService.findOneById(ENTITY_TYPE_META_DATA, entity, EntityType.class);
  ...
  private Entity findAttributeEntity(String entityTypeId, String attributeName) {
    ...
    Optional<Attribute> result =
        stream(entityTypeEntity.getAttributes().spliterator(), false)
            .filter(att -> attributeName.equals(att.getName()))
            .findFirst();
