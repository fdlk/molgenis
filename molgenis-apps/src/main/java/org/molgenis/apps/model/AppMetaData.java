...
public class AppMetaData extends SystemEntityType {
  ...
  @Autowired
  public AppMetaData(
      FileMetaMetaData fileMetaMetaData, FreemarkerTemplateMetaData freemarkerTemplateMetaData) {
  ...
  protected void init() {
    ...
    addAttribute(DESCRIPTION, ROLE_LOOKUP)
        .setDataType(TEXT)
        .setNillable(true)
        .setLabel("Description");
    addAttribute(ICON_HREF)
        .setDataType(HYPERLINK)
        .setNillable(true)
        .setLabel("Icon URL")
        .setDescription("Absolute or relative URL of the app icon");
    addAttribute(RESOURCE_ZIP)
        .setDataType(FILE)
    ...
    addAttribute(LANDING_PAGE_HTML_TEMPLATE)
        .setDataType(XREF)
        ...
        .setValidationExpression(
            "$('"
                + LANDING_PAGE_HTML_TEMPLATE
                + "').value() != null || $('"
                + USE_FREEMARKER_TEMPLATE
                + "').eq(false).value()")
